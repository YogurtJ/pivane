...
async function run(browser, width, locale, theme) {
    ...
    await page.locator('#native-package-install:not([disabled])').waitFor();
    ...
    await page.locator('.native-resource-origin summary').click(); await check();
    assert.equal(await page.locator('.native-resource-origin code').textContent(), resources[45].path);
    ...
    assert.ok(release); await page.locator('#native-resource-scope').selectOption('global'); await page.locator('#native-package-install:not([disabled])').waitFor();
