...
右侧历史页增加“搜索 / 会话树”两个入口；`/tree`（无参数）与线程菜单“会话树”打开相同视图。树默认定位当前继续位置，以纵向分支连线呈现问题、回复、书签和两类摘要。纯工具、思考和内部 metadata 不铺进树；工具仍可在搜索中查找。可收起后续路径、定位当前和分页，每页最多100个节点，深层分支缩进有上限，父节点预览在按钮提示中。原生 parentId 关系经过不可见节点连接后投影，不重新推算或保存聊天树。
...
入口为线程菜单“搜索历史与书签”和右侧“历史”页签；会话详情不再重复放置入口。历史顶部“搜索／会话树”同排右侧的更多（⋯）只收纳“从历史问题分叉”和“恢复旧版本”，复用原工作流窗口与确认，不增加常驻操作卡片。菜单不挤压列表、预览正文，不触发额外历史读取；外部点击、Escape、切页或切线程收起，键盘关闭返回入口。分叉到新线程与会话树的原线程继续保持不同语义。此入口调整为静态刷新生效。
