...
桌面点击顶部模型名称（或输入 `/model`）打开选择面板；项目入口使用文件夹图标，状态行同时显示线程标题和连接状态。手机在输入框发送按钮左侧点击紧凑上下文圆环进入模型/思考/上下文面板，环内仅显示数字（`35` 表示 35%）；思考等级使用无描边的浅底选择控件。再点“模型”打开可搜索选择器；输入框聚焦时还显示一行当前模型、思考等级和完整百分比摘要。模型切换后的思考选项按新模型实际支持的等级更新，忙碌时不可切换。项目目录在手机顶栏文件夹入口直接选择或添加；文件、历史以及“正文/完整记录”在 `i` 详情页，不混入模型面板。上下文尚无实际统计或压缩后等待下一次回复时显示 `--` 而不是 0%。默认优先展示加星的常用模型，再补充当前模型和最近使用，重复项只显示一次；首次使用无需预先整理目录。点击模型行切换当前会话，点击星标只加入或取消常用，不发送请求。

搜索同时匹配名称、完整模型 ID 和供应商，覆盖整个当前可用目录；“查看全部模型”按供应商分组浏览，每批显示 60 项，可继续显示更多。每行保留供应商与完整 ID，来自不同渠道的同名模型分别收藏。支持 Tab、上下方向键、Enter 和 Escape；手机使用底部面板，搜索区固定、结果内部滚动。

常用按添加顺序排列，保存到此 Pivane 实例，在 Mac、手机及其他设备间共享。打开选择器、回到页面时读取服务器最新状态，面板保持打开时每 5 秒核对一次；更新只提交单个模型的加星或取消，不覆盖其他设备的整份列表。读取或保存失败会明确提示，不自动重放不确定的加星操作。

升级前已有的浏览器收藏，在该浏览器加载新版并打开会话或选择器后合并到实例；旧浏览器副本保留。局域网普通 HTTP 页面使用浏览器可用的加密随机数生成迁入 ID，不依赖安全上下文专有的 `crypto.randomUUID()`。浏览器的“已迁入”标记不能单独证明服务器仍保留回执，因此每次加载页面会用稳定 ID 核对一次；重复导入不会改动列表或修订，取消标记阻止迟到设备恢复已取消的收藏。请先在保存旧收藏的原浏览器、原网址刷新，再在其他设备打开选择器；不同网址的浏览器存储不互通。

最近使用最多保留 5 项，仍属于当前浏览器，记录打开会话或成功切换后的可用模型。常用与最近都不修改全局默认模型、认证或原生会话历史。暂不可用的收藏不会加入可选列表，恢复可用后重新显示；当前会话模型仍保留名称。模型刷新、会话忙碌或断开时关闭面板并禁用切换，失败或迟到结果不会记为新选择。手机／触控设备打开面板时焦点在关闭按钮，点击搜索框后才聚焦输入；桌面保留自动聚焦搜索。
...
子 Agent 设置和辅助模型也使用这一原生目录，而会话可能加载扩展注册或覆盖的模型。同一供应商同时写在扩展与 `models.json` 中时，修改其中一份不会自动改写另一份；发现名称或型号不同，应核对两份配置，而非只重启会话。设置页每次进入“供应商与模型”或“使用偏好”都会重新读取本地目录，保留辅助模型草稿；显式“刷新模型目录”才会请求供应商远端目录。
