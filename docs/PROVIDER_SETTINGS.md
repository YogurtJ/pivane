...
“使用偏好”的[辅助模型](AUXILIARY_MODELS.md)集中管理标题生成和媒体规划（原模块 Agent），并保留独立的回复朗读配置。模型测试发送真实最小请求，最多 64 个输出 token，可能产生费用。配置保存不自动发测试请求。Model ID 支持 `/`、`:`、`@` 等真实供应商标识，按原值查找并编码到 REST 路径，不作为文件路径。
