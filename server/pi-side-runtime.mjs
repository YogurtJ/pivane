...
try {
    ...
    const createRuntime = async ({ cwd, sessionManager, sessionStartEvent }) => {
        ...
        const result = await createAgentSessionFromServices({ services, sessionManager, sessionStartEvent, model,
            thinkingLevel: seed.thinkingLevel, noTools: 'all' });
        if (result.session.sessionFile || result.session.agent.state.tools.length) throw new Error('Side isolation check failed');
