...
class PiSessionSearch {
    ...
    async search(query, signal) {
        ...
        if (Object.keys(query).some(k => !['q', 'cwd', 'offset', 'searchId'].includes(k)) || typeof query.q !== 'string' || query.q.trim().length < 2 || query.q.length > 200) throw fail('请输入 2–200 个字符的关键词');
        ...
        const key = JSON.stringify([q, cwd, this.preferences.getHiddenProjects()]);
        ...
        if (offset) {
            ...
            try {
                ...
                const data = await new Promise((resolve, reject) => {
                    const worker = new Worker(path.join(__dirname, 'pi-session-search-worker.js'), { workerData: { root: path.join(getAgentDir(), 'sessions'), roots: this.store.roots, q, cwd, hidden: this.preferences.getHiddenProjects() }, resourceLimits: { maxOldGenerationSizeMb: 192 } });
                    let done = false;
