(() => {
    ...
    const examples = [
        ['review-changes', translateUi("检查当前改动"), translateUi("检查当前项目的改动，关注正确性、回归风险与测试缺口。先阅读项目约定；没有 Git 时根据当前文件与可用备份核对，不假设存在 Git。\n额外要求：${1:-优先指出需要修复的问题}")],
        ['finish-task', translateUi("按项目约定收尾"), translateUi("按当前项目的 AGENTS.md 和开发约定完成收尾：核对改动、运行相关验证、更新必要文档，并说明结果及尚未完成的事项。\n范围：${1:-本轮任务}")],
        ['explain-file', translateUi("解释选中的文件"), translateUi("请读取并解释 $1，说明职责、关键流程、依赖与值得注意的边界。\n${2:-使用简洁中文，结合具体代码说明}")],
        ['handoff', translateUi("生成交接说明"), translateUi("为${1:-当前任务}生成交接说明，包含目标、已完成改动、验证证据、仍待处理事项和关键文件路径；不要包含凭据。")]
    ];
    class PiComposerTools {
        constructor(host) {
            ...
            this.menu.setAttribute('role', 'listbox'); this.menu.setAttribute('aria-label', translateUi("命令与文件补全"));
        ...
        setEnabled(value) {
            ...
            $('settings-manage-templates').title = value ? translateUi("管理 Pi 原生提示词模板") : translateUi("模板管理等待后台启用");
        }
        ...
        show(items) {
            ...
            if (!items.length) { this.menu.append(node('p', translateUi("没有匹配项"), 'pi-composer-empty')); }
            items.forEach((item, i) => {
                ...
                b.append(node('strong', c ? `/${c.name}` : `@${item.file.path}`), node('span', c ? `${this.source(c)} · ${c.argumentHint || ''} ${c.description || ''}` : translateUi("项目文件路径引用")));
                this.menu.append(b);
            });
            this.menu.classList.remove('hidden'); this.input.setAttribute('aria-expanded', 'true');
        }
        source(c) { return { builtin: translateUi("内置命令"), web: translateUi("网页操作"), prompt: translateUi("模板"), skill: 'Skill', extension: translateUi("扩展") }[c.source] || c.source || translateUi("命令"); }
        handleKey(e) {
        ...
        openTemplates() {
            if (!this.host.context().cwd) return this.host.toast(translateUi("请先选择项目，再管理全局或项目模板"), 'info');
            this.hide(); this.closeActions(); this.search.value = '';
            this.body.replaceChildren(node('p', translateUi("正在加载…")));
        ...
        async renderPanel() {
            ...
            if (!this.enabled) { this.body.append(node('p', translateUi("模板管理暂不可用，请等待后台启用。"))); return; }
            const actions = node('div', null, 'pi-composer-actions-row');
            const reload = this.button(translateUi("重新加载当前会话资源"), () => this.reload()); reload.disabled = !this.host.context().connected;
            actions.append(this.button(translateUi("新建模板"), () => this.editor()), reload);
            this.body.append(actions, node('p', translateUi("模板是按需调用的任务快捷方式：在聊天输入框输入 /名称 和参数才会展开，不会自动加入每轮对话。长期项目约定放在 AGENTS.md，README 用于项目说明。"), 'pi-composer-help'));
            this.body.append(node('p', translateUi("与终端共用原生 prompts 目录。保存后，已打开的会话需空闲时重新加载；项目模板受 Pi trust 控制。覆盖与删除会留备份，同名模板被遮蔽时需改名。"), 'pi-composer-help'));
                const sample = node('details'); sample.append(node('summary', translateUi("从常用任务开始")));
                ...
                for (const t of this.templates.filter(t => `${t.name} ${t.description}`.toLowerCase().includes(query))) {
                    const row = node('div', null, 'pi-composer-template'); row.append(node('strong', `/${t.name} · ${t.scope === 'user' ? translateUi("全局") : translateUi("当前项目")}`), node('small', t.description));
                    const c = this.commands().find(c => c.source === 'prompt' && (c.path || c.sourceInfo?.path) === t.filePath);
                    row.append(this.button(translateUi("编辑"), async () => { const key = this.key(), request = ++this.request; const record = await this.host.api(`/api/pi/composer/template?cwd=${encodeURIComponent(this.host.context().cwd)}&scope=${t.scope}&name=${encodeURIComponent(t.name)}`); if (key === this.key() && request === this.request && this.dialog.open) this.editor(record); }));
                    row.append(node('small', c ? translateUi("已加载，在输入框输入 /{0}{1} 使用", c.name, c.argumentHint ? ' ' + c.argumentHint : '') : translateUi("尚未加载到当前会话；打开会话或重新加载后使用")));
                    row.append(this.button(translateUi("删除"), async () => {
                        this.valid(); if (!window.confirm(translateUi("删除 {0}/{1}？旧文件保留在模板备份中。", t.scope, t.name))) return;
        ...
        editor(record = {}) {
            this.request++;
            this.search.hidden = true; this.body.replaceChildren(this.button(translateUi("返回模板列表"), () => this.openTemplates()), node('h3', record.revision ? translateUi("编辑模板") : translateUi("新建模板")));
            const form = node('form');
            const nameLabel = node('label', translateUi("命令名（字母、数字、-、_；不含 / 和 .md）")), name = node('input'); name.value = record.name || ''; name.required = true; name.pattern = '[a-zA-Z0-9][a-zA-Z0-9_-]{0,79}'; name.disabled = Boolean(record.revision); name.id = 'pi-template-name'; nameLabel.append(name);
            const scopeLabel = node('label', translateUi("保存位置")), scope = node('select'); scope.id = 'pi-template-scope';
            for (const [value, text] of [['user', translateUi("全局 · 与所有项目共用")], ['project', translateUi("当前项目 · 需 Pi 信任后加载")]]) { const option = node('option', text); option.value = value; scope.append(option); }
            scope.value = record.scope || 'user'; scope.disabled = Boolean(record.revision); scopeLabel.append(scope);
            const contentLabel = node('label', translateUi("原生 Markdown 模板")), content = node('textarea'); content.id = 'pi-template-content'; content.rows = 12; content.required = true; content.value = record.content || translateUi("---\ndescription: 简要说明此模板的用途\nargument-hint: \"[补充要求]\"\n---\n请完成以下任务：\n${1:-补充默认要求}\n"); contentLabel.append(content);
