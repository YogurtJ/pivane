(() => {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        async function api(path = '', body, method = body ? 'POST' : 'GET') {
            ...
            catch { throw new Error('连接中断；请刷新服务列表核对是否已保存。'); }
        ...
        async function action(element, task) {
            if (busy) { message('上一次接入操作仍在进行，请稍候。'); return; }
            const view = epoch;
        ...
        async function showOverview() {
            const view = shell('媒体服务与模型'); message('正在读取服务');
            try { await reload(); if (!dialog.open || view !== epoch) return; renderOverview(); }
            catch (error) { if (dialog.open && view === epoch) { content.replaceChildren(); message(error.message, true); footer.append(button('重试', showOverview)); } }
        }
        function keyLabel(provider) {
            return provider.auth.mode === 'none' ? '无需 Key' : provider.keyNeedsRebind ? '地址已更换 · 需要重新保存 Key' : provider.keyConfigured ? 'Key 已保存' : '尚未配置 Key';
        }
        function renderOverview() {
            shell('媒体服务与模型');
            message('选择服务，添加自己的图像、视频或语音模型。');
            if (!snapshot.providers.length) message('还没有媒体服务。从常用服务开始，或接入自己的兼容 API。');
            ...
            for (const provider of snapshot.providers) {
                ...
                actions.append(button(`管理模型（${provider.models.length}）`, () => showProvider(provider.id), true), button('编辑服务', () => editProvider(provider.id)));
                card.append(actions); list.append(card);
            }
            content.append(list); footer.append(button('刷新列表', showOverview), button('新增服务', () => editProvider(), true));
        }
        ...
        function showProvider(id) {
            ...
            const probe = button('连接测试', () => action(probe, async () => {
            ...
            const discover = button('读取模型列表', () => action(discover, async () => {
            ...
            probes.append(probe, discover, button('编辑服务 / Key', () => editProvider(id))); content.append(probes, result);
            ...
            for (const model of provider.models) {
                ...
                const actions = node('div', 'mc-actions');
                const remove = button('删除模型', () => action(remove, async () => {
                    if (!confirm(`删除“${model.name}”的接入配置？生成记录和媒体文件会保留。`)) return;
                ...
                actions.append(button('使用', () => { changed(`media:${id}:${model.id}`); dialog.close(); }, true), button('编辑模型', () => editModel(id, model.id)), remove);
                row.append(actions); list.append(row);
            }
            if (!provider.models.length) message('还没有配置可生成的媒体模型。可以手动填写模型 ID，或读取服务提供的模型列表。', false, list);
            content.append(list); footer.append(button('添加模型', () => editModel(id), true));
        }
        function editProvider(id) {
            const existing = id ? providerById(id) : null;
            const view = shell(existing ? '编辑媒体服务' : '新增媒体服务', () => existing ? showProvider(id) : renderOverview());
            ...
            if (!existing && snapshot.providerTemplates?.length) {
                ...
                const choose = field(presets, '常用服务', 'select', '', { choices: [{ value: '', label: '自定义 / 兼容 API' }, ...snapshot.providerTemplates.map(item => ({ value: item.id, label: item.name }))], help: '选好服务后填入 Key；地址可改为自己的代理或区域地址。' }).input;
            ...
            const fields = {};
            fields.name = field(form, '服务名称', 'text', existing?.name, { required: true, maxLength: 200 }).input;
            const internalId = field(form, '服务 ID', 'text', existing?.id || `service-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`, { required: true, readOnly: Boolean(existing), maxLength: 80 }); fields.id = internalId.input;
            ...
            fields.auth = field(form, '认证方式', 'select', existing?.auth.mode || 'bearer', { choices: [{ value: 'bearer', label: 'Bearer API Key' }, { value: 'header', label: '自定义 Key Header' }, { value: 'none', label: '无需 Key' }] }).input;
            const key = field(form, 'API Key', 'password', '', { maxLength: 32768, help: existing ? keyLabel(existing) + '；留空保留已保存的 Key。' : 'Key 仅保存到此实例，不会回显。' });
            key.input.autocomplete = 'new-password';
            const header = field(form, 'Key Header 名称', 'text', existing?.auth.header || 'x-api-key', { maxLength: 80 });
            const prefix = field(form, 'Key 前缀（可留空）', 'text', existing?.auth.prefix || '', { maxLength: 40, placeholder: '例如 Token 后加一个空格' });
            const details = node('details', 'mc-details wide'); details.append(node('summary', '', '高级连接设置'));
            const advanced = node('div', 'lab-fields');
            fields.probe = field(advanced, '连接测试路径（GET）', 'text', existing?.probePath ?? '/models', { maxLength: 2000 }).input;
            fields.models = field(advanced, '模型列表路径（GET，可留空）', 'text', existing?.modelsPath ?? '/models', { maxLength: 2000 }).input;
            fields.origins = field(advanced, '额外下载域名', 'textarea', (existing?.downloadOrigins || []).join('\n'), { wide: true, rows: 3, help: '每行一个完整来源，例如 https://cdn.example.com。服务本身的来源已允许；Key 不会发送到其他来源。' }).input;
            ...
            const save = button('保存服务', () => action(save, async () => {
                ...
                if (!confirm(`保存媒体服务“${provider.name}”（${provider.baseUrl}）？确认生成时，参数将发送到此服务。`)) return;
                ...
                try {
                    ...
                    if (providerSaved) {
                        ...
                        if (view === epoch) { editProvider(provider.id); message('服务已保存；Key 状态需要核对。' + error.message, true); }
                    } else throw error;
                }
            }), true);
            footer.append(save);
            if (existing) {
                const removeKey = button('移除 Key', () => action(removeKey, async () => {
                    if (!confirm('移除这个媒体服务保存的 Key？')) return;
                    await api(`/providers/${encodeURIComponent(id)}/key`, stamp(), 'DELETE'); await reload(); changed(); if (view === epoch) editProvider(id);
                }));
                const remove = button('删除服务', () => action(remove, async () => {
                    if (!confirm(`删除“${existing.name}”、其接入模型和 Key？生成记录及媒体文件会保留。`)) return;
        ...
        function editModel(providerId, modelId, initial = {}, preservedDocs = '', notice = '') {
            ...
            const view = shell(existing ? '编辑媒体模型' : '添加媒体模型', () => showProvider(providerId));
            let formRevision = 0, reading = false;
            const form = node('form', 'lab-fields'); form.addEventListener('submit', event => { event.preventDefault(); save.click(); });
            form.addEventListener('input', () => { formRevision++; });
            const fields = {};
            fields.name = field(form, '显示名称', 'text', existing || initial.http ? current.name : '', { placeholder: '可留空，使用模型 ID', maxLength: 200 }).input;
            fields.remote = field(form, '模型 ID（服务端）', 'text', current.remoteModel, { required: true, maxLength: 500 }).input;
            fields.kind = field(form, '媒体类型', 'select', current.kind, { choices: Object.entries(labels).map(([value, label]) => ({ value, label })) }).input;
            const templates = [{ value: '', label: '自定义 / 保留当前配置' }, ...[...snapshot.templates].sort((a, b) => Number(Boolean(b.recommended)) - Number(Boolean(a.recommended))).map(template => ({ value: template.id, label: template.name }))];
            const matchedTemplate = snapshot.templates.find(item => JSON.stringify(item.model.http) === JSON.stringify(current.http));
            fields.template = field(form, '协议模板', 'select', matchedTemplate?.id || '', { choices: templates }).input;
            form.prepend(fields.kind.closest('label'), fields.template.closest('label'));
            const protocolSummary = node('p', 'mc-protocol-summary wide', notice || matchedTemplate?.help || '常用协议已填好。填写服务端模型 ID 后即可保存，特殊参数可交给 Agent 配置。');
            ...
            fields.kind.addEventListener('change', () => {
                ...
                editModel(providerId, modelId, { ...clone(selected.model), id: current.id, name: fields.name.value, remoteModel: fields.remote.value }, docs.input.value, selected.help || '已切换媒体类型，请核对模型 ID。');
            });
            fields.template.addEventListener('change', () => {
                ...
                editModel(providerId, modelId, { ...clone(selected.model), id: current.id, name: fields.name.value, remoteModel: fields.remote.value }, docs.input.value, selected.help || '模板已填入，可根据所选模型的要求调整。');
            });
            const agentBox = node('details', 'mc-details wide'); agentBox.append(node('summary', '', '让 Agent 根据 API 文档生成接入草稿'));
            const docs = field(agentBox, 'API 文档或请求/响应示例', 'textarea', preservedDocs, { rows: 6, maxLength: 64000, help: '粘贴或选择文档文件，不要包含 Key。Agent 只生成配置草稿，保存和生成由你确认。' });
            const fileLabel = node('label', 'mc-file', '选择文档文件');
            const file = node('input'); file.type = 'file'; file.accept = '.md,.txt,.json,.yaml,.yml,.html'; file.setAttribute('aria-label', '选择 API 文档文件'); fileLabel.append(file); agentBox.append(fileLabel);
            const draftStatus = node('p', 'lab-muted');
            const plan = button('生成接入草稿', () => action(plan, async () => {
                ...
                if (epoch !== view || !dialog.open || formRevision !== captured) { if (dialog.open && epoch === view) draftStatus.textContent = '编辑内容已变化，未覆盖当前草稿。'; return; }
                ...
                editModel(providerId, modelId, { ...result.draft.model, id: current.id || result.draft.model.id }, docs.input.value,
                    `${result.draft.summary} · ${result.plannerModel.name}${result.fallbackUsed ? '（备用模型）' : ''}。` + result.draft.warnings.join('；') + ' 尚未保存，可继续修改。');
            }));
            let fileRevision = 0;
            file.addEventListener('change', async () => {
                ...
                try {
                    if (selected.size > 64000) throw new Error('文档文件请控制在 64KB 以内。');
                ...
                } catch (error) { if (view === epoch) draftStatus.textContent = error.message || '文档必须是 UTF-8 文本。'; }
                finally { if (token === fileRevision) { reading = false; plan.disabled = false; } }
            });
            agentBox.append(plan, draftStatus); form.append(agentBox);
            const protocol = node('details', 'mc-details wide'); protocol.append(node('summary', '', '高级：接口、输出与轮询'));
            const protocolFields = node('div', 'lab-fields'); protocol.append(protocolFields);
            fields.path = field(protocolFields, '生成接口路径（POST）', 'text', current.http.path, { required: true, wide: true, help: `追加到 ${provider.baseUrl} 后，不重复其 /v1 前缀。可用 {model} 或 {param:voice} 插入模型/参数。`, maxLength: 2000 }).input;
            fields.response = field(protocolFields, '结果形式', 'select', current.http.response.type, { choices: [{ value: 'image-json', label: '标准图像响应（自动识别）' }, { value: 'base64', label: 'JSON 中的 base64' }, { value: 'url', label: 'JSON 中的下载 URL' }, { value: 'binary', label: '直接返回媒体文件' }] }).input;
            fields.mime = field(protocolFields, '文件格式', 'select', current.http.response.mimeType, { choices: ['auto','image/png','image/jpeg','image/webp','video/mp4','audio/wav','audio/mpeg'] }).input;
            const output = field(protocolFields, '结果字段路径', 'text', pathText(current.http.response.path), { wide: true, placeholder: 'data.0.b64_json', help: '用点分隔字段和数组序号，也可填写 JSON 路径数组。', maxLength: 2000 });
            const asynchronous = field(protocolFields, '异步任务：提交后查询状态', 'checkbox', Boolean(current.http.poll), { wide: true });
            ...
            fields.idPath = field(polling, '任务 ID 字段', 'text', pathText(poll.idPath), { maxLength: 2000 }).input;
            fields.pollMode = field(polling, '查询地址来源', 'select', poll.urlPath ? 'response' : 'path', { choices: [{ value: 'path', label: '路径模板' }, { value: 'response', label: '提交响应中的 URL' }] }).input;
            const pollPath = field(polling, '查询路径（GET）', 'text', poll.path || '/tasks/{id}', { wide: true, help: '使用 {id} 插入任务 ID；查询始终位于当前服务来源。', maxLength: 2000 });
            const pollUrl = field(polling, '查询 URL 字段', 'text', pathText(poll.urlPath || ['urls','get']), { wide: true, maxLength: 2000 });
            fields.statusPath = field(polling, '任务状态字段', 'text', pathText(poll.statusPath), { maxLength: 2000 }).input;
            fields.interval = field(polling, '查询间隔（秒）', 'number', poll.intervalMs / 1000, { min: 1, max: 60, step: 1 }).input;
            for (const [key, label] of [['pending','处理中状态'],['succeeded','成功状态'],['failed','失败状态']]) fields[key] = field(polling, label, 'text', poll[key].every(value => typeof value === 'string') ? poll[key].join(', ') : JSON.stringify(poll[key]), { wide: true, help: '多个字符串用英文逗号分隔；布尔值或数字用 JSON 数组，如 [true] 或 [false,null]。', maxLength: 2000 }).input;
            protocolFields.append(polling);
            fields.timeout = field(protocolFields, '本次请求最长等待（秒）', 'number', current.http.timeoutMs / 1000, { min: 1, max: 1800, step: 1, help: '超时不代表远端任务已取消；不会自动重新提交。', wide: true }).input;
            const advanced = node('details', 'mc-details wide'); advanced.append(node('summary', '', '高级：参数定义与请求映射'));
            fields.parameters = field(advanced, '参数定义 JSON', 'textarea', JSON.stringify(current.parameters, null, 2), { rows: 10, code: true, maxLength: 64000 }).input;
            fields.body = field(advanced, '请求体模板 JSON', 'textarea', JSON.stringify(current.http.body, null, 2), { rows: 8, code: true, maxLength: 32000, help: '{$param:"字段"} 引用参数；{$model:true} 引用模型 ID；{$params:true} 引用全部参数。' }).input;
            form.append(protocol, advanced);
            fields.instructions = field(advanced, '模型使用要求', 'textarea', current.instructions, { rows: 3, wide: true, maxLength: 64000, help: '生成方案时，Agent 会读取这些要求。' }).input;
            content.append(form);
            ...
            const save = button('保存并使用模型', () => action(save, async () => {
                if (!valid(form)) return;
                ...
                if (!confirm(`保存“${model.name}”的接入配置到“${provider.name}”？这一步不会生成媒体。`)) return;
