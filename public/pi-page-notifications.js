(() => {
    ...
    function render() {
        ...
        $('pi-page-status').textContent = !supported()
            ...
            : !canNotify() ? 'iPhone / iPad 的系统通知需从主屏幕应用启用；这里仍可使用提示音。'
            : Notification.permission === 'denied' ? '系统通知权限已拒绝，请在浏览器网站设置中恢复。'
            : push ? '后台推送已启用，页面不重复发送系统提醒。'
            : prefs.desktop ? '页面打开时接收系统提醒；无需连接推送服务。' : '本机 localhost 可直接开启；站内未读提醒始终保留。';
    }
    function save() { try { localStorage.setItem(key, JSON.stringify(prefs)); } catch {} render(); }
    async function sound(unlock = false) {
        if (!audio && !unlock) throw Error('提示音需要解锁，请点击测试提醒。');
        audio ||= new (window.AudioContext || window.webkitAudioContext)();
        if (audio.state !== 'running' && unlock) {
            await Promise.race([audio.resume(), new Promise((_, reject) => setTimeout(() => reject(Error('声音尚未解锁，请再次点击测试提醒。')), 1500))]);
        }
        if (audio.state !== 'running') throw Error('浏览器暂停了声音，请点击测试提醒后重试。');
    ...
    async function systemNotice(body, tag) {
        if (!canNotify() || Notification.permission !== 'granted') throw Error('请先允许浏览器系统通知。');
        const mobile = /Android|iPad|iPhone|iPod/.test(navigator.userAgent) || ios();
        if (mobile) {
            if (!navigator.serviceWorker) throw Error('此浏览器不支持页面系统通知。');
            let reg = await navigator.serviceWorker.getRegistration('/');
            const expected = new URL('/pi-notification-sw.js', location.href).href;
            if (reg && ![reg.active, reg.waiting, reg.installing].some(worker => worker?.scriptURL === expected)) throw Error('当前地址已有其他应用的通知服务。');
            if (!reg) {
                await navigator.serviceWorker.register('/pi-notification-sw.js', { scope: '/', updateViaCache: 'none' });
                reg = await Promise.race([navigator.serviceWorker.ready, new Promise((_, reject) => setTimeout(() => reject(Error('通知服务启动超时')), 15000))]);
            }
    ...
    async function notify(body, id, test = false, claimed = false) {
        ...
        if (failed) $('pi-page-status').textContent = failed.reason.message || '通知显示失败，请检查浏览器权限。';
        else if (test) $('pi-page-status').textContent = jobs.length ? '测试提醒已触发，请检查系统通知或声音。' : '请先开启页面通知或提示音。';
    }
    async function ingest(data, generation) {
        ...
        for (const n of notices) {
            ...
            if (Date.parse(n.completedAt) >= enabledAt) events.push(['新的回复已完成，点击查看。', n.completionId]);
        }
        for (const [id, phase] of phases) {
            if (previous.has(id) && previous.get(id) !== phase && ['waiting', 'error'].includes(phase)) {
                events.push([phase === 'waiting' ? '任务正在等待你的确认。' : '任务未成功完成，请打开工作台查看。', 'state-' + Date.now()]);
            }
        }
        previous = phases;
        ...
        if (events.length) await notify(events.length > 1 ? `${events.length} 项任务有新的提醒，请查看工作台。` : events[0][0], events[0][1]);
    }
    async function poll() {
    ...
    $('pi-page-sound').addEventListener('change', event => {
        ...
        if (prefs.sound) void sound(true).catch(() => { $('pi-page-status').textContent = '声音尚未解锁，请点击测试提醒。'; });
        void poll();
    });
    $('pi-page-test').addEventListener('click', () => void notify('这是一条页面提醒，无需推送服务。', 'test', true));
    ...
    window.PiPageNotifications = {
        observeCompletion(notice) {
            ...
            while (seen.size > 512) seen.delete(seen.values().next().value);
            void notify('新的回复已完成，点击查看。', notice.completionId);
        },
