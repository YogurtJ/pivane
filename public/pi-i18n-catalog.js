...
,['读取技能 · {0}', 'Read skill · {0}']
    ...
    ,['进行中', 'In progress']
    ,['全部完成', 'All completed']
    ,['已完成 {0}/{1} 步', '{0} of {1} steps completed']
    ,['更新任务计划', 'Update task plan']
    ,['预览更新提醒', 'Preview update reminder']
    ,['提醒样式预览', 'Update reminder preview']
    ,['示例版本，仅预览样式；不会检查更新或修改提醒设置。', 'Example versions for a visual preview. No update check or reminder settings will be changed.']
    ,['收起预览', 'Hide preview']
    ,['这是样式预览，未执行任何操作。', 'This is a visual preview. No action was performed.']
    ,['可在任务结束后更新。', 'Update when your tasks are done.']
    ,['显示隐藏文件', 'Show hidden files']
    ,['查找文件名或路径', 'Find a file or path']
    ,['项目文件', 'Project files']
    ,['选择项目后即可浏览文件', 'Select a project to browse its files']
    ,['当前后端尚未启用项目文件浏览', 'Project file browsing is not available on this backend']
    ,['已展开较多目录，请刷新目录后继续', 'Many folders are open. Refresh the tree to continue.']
    ,['正在读取目录…', 'Loading folder…']
    ,['空目录', 'Empty folder']
    ,['正在查找文件…', 'Finding files…']
    ,['仅显示部分结果，请缩小搜索范围', 'Showing partial results. Refine your search.']
    ,['找到 {0} 个文件', '{0} files found']
    ,['未找到匹配文件', 'No matching files']
    ,['目录内容较多，仅显示部分条目', 'Showing part of this large directory']
    ,['此目录没有可显示的文件', 'No visible files in this folder']
    ,['浏览项目文件', 'Browse project files']
    ,['展开阅读', 'Expand reader']
    ,['文件路径', 'File path']
    ,['复制文件路径', 'Copy file path']
    ,['已复制文件路径', 'File path copied']
    ,['在项目中定位', 'Reveal in project']
    ,['项目根目录', 'Project root']
    ,['收起阅读区', 'Collapse reader']
    ,['目录正在变化，请刷新后重试', 'The folder changed while being read. Refresh to try again.']
    ,['文件浏览参数无效', 'Invalid file browsing parameters']
    ,['文件浏览正在进行，请稍后再试', 'File browsing is busy. Please try again shortly.']
    ,['请选择一个目录', 'Please select a folder']
    ,['文件浏览已取消', 'File browsing was cancelled']
    ,['项目目录无效', 'Invalid project directory']
    ,['搜索模型或供应商…', 'Search models or providers…']
    ,['点击星标固定常用模型，偏好保存在当前浏览器。', 'Star models to keep them handy. Preferences are saved in this browser.']
    ,['浏览器无法保存模型偏好，本次页面内仍可使用。', 'Model preferences could not be saved. They remain available on this page.']
    ,['显示 {0} / {1} 个模型', 'Showing {0} of {1} models']
    ,['没有匹配的模型，请尝试其他名称或供应商。', 'No matching models. Try another name or provider.']
    ,['常用模型', 'Favorites']
    ,['当前模型', 'Current model']
    ,['最近使用', 'Recently used']
    ,['搜索或查看全部模型，点击星标加入常用。', 'Search or browse all models, then star your favorites.']
    ,['返回常用模型', 'Back to favorites']
    ,['查看全部模型（{0}）', 'View all models ({0})']
    ,['取消常用：{0}（{1}）', 'Remove favorite: {0} ({1})']
    ,['加入常用：{0}（{1}）', 'Add favorite: {0} ({1})']
    ,['正在预约…', 'Scheduling…']
    ,['正在保存预约…', 'Saving schedule…']
    ,['正在回退并发送…', 'Rewinding and sending…']
    ,['正在恢复…', 'Restoring…']
    ,['正在创建线程…', 'Creating thread…']
    ,['正在创建分叉…', 'Creating fork…']
    ,['正在发送…', 'Sending…']
    ,['正在取消…', 'Cancelling…']
    ,['正在暂停…', 'Pausing…']
    ,['正在删除…', 'Deleting…']
    ,['正在处理…', 'Processing…']
    ,['已预约发送', 'Message scheduled']
    ,['已暂停发送', 'Scheduled message paused']
    ,['操作已完成，列表刷新失败，请刷新查看', 'Operation completed, but the list could not refresh. Refresh to view the result.']
    ,['新线程已创建，打开失败，请在线程列表中查看', 'The new thread was created but could not be opened. Find it in the thread list.']
    ,['常用模型在此实例的设备间共享；最近使用保存在当前浏览器。', 'Favorites are shared across devices on this instance; recent models stay in this browser.']
    ,['旧收藏尚未确认同步，请重新打开选择器核对。', 'Legacy favorites have not been confirmed as synced. Reopen the picker to check.']
    ,['常用模型同步暂不可用，请重新打开选择器核对。', 'Favorites sync is temporarily unavailable. Reopen the picker to check.']
    ,['正在保存常用模型…', 'Saving favorites…']
    ,['常用模型未确认保存，请重新打开选择器核对；未自动重试。', 'Saving favorites was not confirmed. Reopen the picker to check; the write was not retried.']
];
