(() => {
    ...
    function create({ apiFetch, context, resources, reload, configuration, restart }) {
        ...
        const checkConfig = button('核对配置', 'pi-runtime-config-check', () => void inspectConfiguration());
        const restartButton = button('重新打开运行实例', 'pi-runtime-restart', async () => {
            if (!configValue || context().busy || !context().connected) return;
            if (!confirm('重新打开此线程的运行实例以应用已保存的配置？对话与当前草稿保留，延迟发送将暂停。')) return;
            const value = configValue, parent = key(); restartButton.disabled = true;
            try { await restart(value); if (parent === key()) feedback.textContent = '已接受，正在重新打开；请等待连接恢复后核对配置。'; }
        ...
        trustBanner.append(node('span', '此项目的 Skills、扩展与项目设置尚未获准加载。'), button('查看项目信任', '', () => openTrust(context().cwd)), button('暂不处理', '', () => { trustBanner.hidden = true; }));
        ...
        async function inspectConfiguration() {
            ...
            try {
                ...
                configValue = value;
                feedback.textContent = value.matchesSavedConfig === null ? '无法核对本实例的启动配置；可在空闲时重新打开实例后核对。' : value.matchesSavedConfig ? '配置文件与本实例启动时一致。资源文件内容修改后，可重新加载资源。' : '已保存的配置与当前实例不同。重新打开运行实例后应用；仅刷新网页不会生效。';
            ...
            } catch (e) { if (n === configEpoch && parent === key()) { feedback.textContent = '配置状态暂时无法核对：' + e.message; feedback.dataset.state = 'unknown'; } }
        ...
        async function loadTrust(cwd, n, parent) {
            body.replaceChildren(node('p', '正在读取项目信任…'));
            try {
                ...
                const state = node('span', trust.effective ? '已信任' : '未信任', { id: 'native-trust-state', class: 'native-trust-badge', 'data-trusted': String(trust.effective) });
                body.append(state, node('p', '以上为按配置推导的状态；当前运行实例的实际信任请在会话详情查看。'), node('p', '信任后，Pi 可以加载这个项目的设置、Skills 和扩展。扩展可执行代码，请只信任熟悉的来源。'));
                const actions = node('div', undefined, { class: 'native-trust-actions' });
                async function save(decision, b) {
                    if (!trustValid(n, parent)) return;
                    if (decision === true && !confirm(`信任项目 ${cwd}？允许加载其中的扩展和资源。`)) return;
                    ...
                    try {
                        ...
                        if (trustValid(n, parent)) message.textContent = refreshed ? '已保存，新运行实例生效。已有线程请在任务结束后 /quit，再重新打开。' : '已保存，但状态读取失败。请重新读取核对。';
                ...
                const allow = button('信任此项目', 'native-trust-allow', () => save(true, allow)); allow.className = 'native-trust-primary';
                const deny = button('不信任', 'native-trust-deny', () => save(false, deny));
                ...
                const details = node('details', undefined, { class: 'native-trust-details' }); details.append(node('summary', '继承与生效范围'));
                details.append(node('p', trust.savedPath ? '信任决定来自：' + trust.savedPath : '未单独设置，使用默认策略。'));
                const policyNames = { ask: '询问（Web 未决项目暂不加载受信资源）', always: '默认信任', never: '默认不信任' };
                details.append(node('p', '全局默认策略：' + (policyNames[trust.defaultPolicy] || trust.defaultPolicy || '未知')));
                if (snapshot.schema.defaultProjectTrust) details.append(button('设置全局默认策略', 'native-trust-global', () => {
                    dialog.close();
                    window.dispatchEvent(new CustomEvent('workspace:open-settings', { detail: { tab: 'native', setting: 'defaultProjectTrust' } }));
                }));
                details.append(node('p', '默认策略仅在没有其他适用决定时使用，已有项目或父目录决定仍有效。保存不会重启当前会话，项目信任不等同于工具权限隔离。'));
                const reset = button('恢复继承', 'native-trust-reset', () => save(null, reset)); details.append(reset); body.append(details);
                if (trust.override !== null) body.append(node('p', '运行环境已将信任固定为' + (trust.override ? '允许' : '拒绝') + '；这里保存的选择暂不会改变该结果。', { class: 'native-trust-notice' }));
                return true;
            } catch (e) { if (trustValid(n, parent)) { body.replaceChildren(button('重新读取', '', () => loadTrust(cwd, n, parent))); message.textContent = e.message; } return false; }
        }
        ...
        function updateControls() {
            ...
            reloadButton.title = c.busy ? '当前任务结束后可重新加载' : '重新加载已保存的资源配置';
        ...
        async function inventory() {
            ...
            content.replaceChildren(node('p', '正在读取当前会话的资源…'));
            try {
                const value = await resources();
                if (n !== readEpoch || parent !== key() || !context().connected) return;
                content.replaceChildren(node('p', `当前会话的项目资源：${value.projectTrusted ? '已信任' : '未信任'}`, { class: 'native-runtime-trust' }));
                const groups = [
                    ['项目指令', value.contextFiles.map(f => f.path)],
                    ['Skills', value.skills.map(s => `${s.name} · ${s.path}`)],
                    ['命令与模板', value.commands.map(c => `/${c.name} · ${c.source.path}`)],
                    ['工具', value.tools.map(t => `${t.name}${t.active ? '' : '（未启用）'} · ${t.source.path}`)]
                ];
                for (const [title, items] of groups) {
                    ...
                    if (items.length) details.append(list); else details.append(node('p', '没有此类资源')); content.append(details);
                }
                const note = node('details', undefined, { class: 'native-runtime-note' }); note.append(node('summary', '清单说明'), node('p', '这里显示当前会话实际加载的来源。配置修改后可能需要重新加载；只注册 hooks 的扩展不在清单中。'), node('p', `系统提示：${value.systemPrompt.custom ? '自定义' : 'Pi 默认'}${value.systemPrompt.appended ? '，含追加指令' : ''}。`)); content.append(note);
        ...
        reloadButton.addEventListener('click', async () => {
            ...
            const n = ++readEpoch, parent = key(); reading = true; updateControls(); content.replaceChildren(node('p', '正在重新加载资源…'));
            try {
                await reload(); if (n !== readEpoch || parent !== key()) return;
                reading = false; await inventory();
            } catch (e) { if (n === readEpoch && parent === key()) content.replaceChildren(node('p', e.message + '；未自动重试。')); }
            finally { if (n === readEpoch) { reading = false; updateControls(); } }
        });
        ...
        sync(); return { openTrust, sync, inspectConfiguration, resourcesChanged() { if (!reading && content.childElementCount) content.replaceChildren(node('p', '资源已更新，刷新清单查看当前加载情况。')); } };
    }
