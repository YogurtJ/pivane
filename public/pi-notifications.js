(() => {
    ...
    async function api(path, method = 'GET', body) {
        ...
        if (!response.ok) throw Error(response.status === 404 ? '通知后端尚未启用，请等待服务空闲更新后刷新。' : data.error || '通知请求失败');
        return data;
    }
    function availability() {
        if (!window.isSecureContext) return '当前地址是 HTTP，浏览器不允许系统通知。请使用可信 HTTPS 地址（服务器本机 localhost 除外）。';
        if (ios() && !standalone()) return 'iPhone / iPad 需要 iOS 16.4 或更新版本：在 Safari 中将此 HTTPS 网站添加到主屏幕，再从主屏幕打开并启用通知。';
        if (!supported()) return '此浏览器不支持 Web Push，请使用支持通知的新版浏览器。';
        if (Notification.permission === 'denied') return '通知权限已被拒绝。请在浏览器／系统的网站通知设置中允许，再刷新状态。';
        return '';
    }
    async function registration(create = false) {
        ...
        if (existing && ![existing.active, existing.waiting, existing.installing].some(w => w?.scriptURL === new URL('/pi-notification-sw.js', location.href).href)) {
            throw Error('此地址已有其他 Service Worker，无法启用通知。');
        }
        if (existing || !create) return existing;
        await navigator.serviceWorker.register('/pi-notification-sw.js', { scope: '/', updateViaCache: 'none' });
        return Promise.race([navigator.serviceWorker.ready, new Promise((_, reject) => setTimeout(() => reject(Error('通知服务启动超时，请刷新状态后核对。')), 15000))]);
    }
    ...
    async function refresh() {
        ...
        status.textContent = enabled ? '本设备已启用：回复完成、任务失败和等待确认时接收通知。' : '本设备尚未启用。点击启用后，浏览器会请求通知权限。';
    }
    async function run(action) {
        if (busy) return; busy = true; buttons.forEach(button => { button.disabled = true; });
        try { await action(); }
        catch (error) {
            status.textContent = error.message || '通知操作失败，请刷新状态核对。';
    ...
    buttons[2].addEventListener('click', () => void run(async () => {
        const sub = await current(); if (!sub) throw Error('请先启用本设备通知');
        await api('/test', 'POST', { subscription: sub.toJSON() });
        await refresh(); status.textContent = '测试已提交给浏览器推送服务。实际显示还取决于系统通知权限、网络与勿扰模式。';
    }));
    buttons[3].addEventListener('click', () => void run(refresh));
    window.addEventListener('workspace:access-ready', () => void run(refresh));
    window.addEventListener('workspace:access-locked', () => { buttons.forEach(button => { button.disabled = true; }); status.textContent = '请先登录工作台，再启用通知。'; });
