document.addEventListener('DOMContentLoaded', () => {
    ...
    const LOCAL_COMMANDS = [
        { name: 'tree', description: '查看会话分支，选择从哪里继续', source: 'web' },
        { name: 'trust', description: '管理当前项目的信任', source: 'web' },
        { name: 'export', description: '打开当前线程的导出记录窗口', source: 'web' },
        { name: 'import', description: '打开 Pi 会话导入窗口，选择文件与目标项目', source: 'web' },
        { name: 'templates', description: '打开设置中的提示词模板入口', source: 'web' },
        { name: 'copy', description: '复制最近一条 Agent 回复', source: 'web' },
        { name: 'compact', description: '压缩上下文，可附加摘要要求', source: 'web' },
        { name: 'quit', description: '退出当前 Pi runtime', source: 'web' },
        { name: 'btw', description: '打开临时侧聊，可附加问题', source: 'web' }
    ];
    const elements = {
    ...
    const shell = new window.PiShell({
        ...
        changed: () => {
            ...
            if (state.connected && !state.shellBusy && !state.streaming && !state.compacting) {
                setConnection('connected', 'Pi Agent 已连接'); setAgentState('connected', '空闲', state.model?.id || 'Pi Agent');
            }
        }
    });
    ...
    const workflows = new window.PiSessionWorkflows({
        ...
        getDraft: () => {
            if (window.PiShell.parse(elements.input.value)) throw new Error('Shell 命令不支持延迟发送，请在当前会话空闲时直接执行');
            if (state.attachmentReads || state.submittingDrafts.has(state.composerSessionKey)) throw new Error('请等待附件读取或消息投递完成');
            if (state.uncertainDrafts.has(state.composerSessionKey) && !window.confirm('上次发送结果不确定。请先核对会话，重复预约可能重复执行任务。确认仍要预约此草稿？')) throw new Error('已取消预约，草稿已保留');
        ...
        openFork: async result => {
            ...
            if (result.draft) {
                ...
                state.attachmentFiles = result.draft.images.map((image, index) => ({ id: `fork-image-${index}`, kind: 'image', name: `图片 ${index + 1}`, data: image.data, mimeType: image.mimeType, preview: `data:${image.mimeType};base64,${image.data}` }));
    ...
    const sideChat = new window.PiSideChat({
        ...
        insertDraft: text => {
            if (!state.connected || state.attachmentReads) throw new Error('请先连接主会话并等待附件读取完成');
    ...
    const nativeControls = new window.PiRuntimeControls({
        ...
        append: text => {
            if (!state.connected || state.attachmentReads || state.submittingDrafts.has(state.composerSessionKey)) throw new Error('请等待连接、附件读取或消息投递完成');
            const draft = `${elements.input.value}${elements.input.value ? '\n\n' : ''}${text}`;
            attachments.validateDraft(draft, state.attachmentFiles);
            elements.input.value = draft; autoResizeInput();
        }
    ...
    const editorSuggestions = new window.PiEditorSuggestions({
        ...
        accept: (text, mode) => {
            if (!state.connected || state.attachmentReads || state.submittingDrafts.has(state.composerSessionKey)) { toast('请等待附件读取或消息提交结束', 'info'); return false; }
    ...
    const historyView = new window.PiHistoryView({
        ...
        useDraft: (draft, onlyEmpty) => {
            if (!state.connected || state.attachmentReads || state.submittingDrafts.has(state.composerSessionKey) || state.streaming || state.compacting) {
                ...
                throw new Error('请等待连接、任务和附件读取完成后追加');
            }
            if (onlyEmpty && (elements.input.value || state.attachmentFiles.length)) return false;
            const images = draft.images.map((image, i) => ({ id: `tree-${Date.now()}-${i}`, kind: 'image', name: `原问题图片 ${i + 1}`, data: image.data, mimeType: image.mimeType, preview: `data:${image.mimeType};base64,${image.data}` }));
    ...
    const conversationSearch = new window.PiSessionSearch({
        ...
        open: async (row, q) => {
            ...
            if (!session) throw new Error('线程已被移动或删除，请重新搜索');
            if (state.session?.ephemeral && !confirm('打开搜索结果会结束当前临时会话，是否继续？')) return;
    ...
    const composer = new window.PiComposerTools({
        ...
        locals: () => LOCAL_COMMANDS.filter(c => c.name !== 'btw' || sideChat.enabled).map(c =>
            c.name === 'tree' && !historyView.tree.enabled ? { ...c, available: false, description: '当前后端尚未启用会话树' } :
            c.name === 'trust' && !state.nativeSettings ? { ...c, available: false, description: '当前后端尚未启用项目信任设置' } :
            ['import', 'export'].includes(c.name) && !transfer.enabled
                ? { ...c, available: false, description: '当前后端尚未启用会话导入与导出，请启用后刷新页面' } : c),
    ...
    async function apiFetch(url, options = {}) {
        ...
        if (response.status === 401) {
            showTokenDialog();
            throw new Error('需要 Pi Web 访问令牌');
        }
    ...
    async function bootstrap() {
        ...
        setConnection('connecting', '正在连接 Pi Agent');
        try {
    ...
    function getProjectName(cwd) {
        return String(cwd || '').split('/').filter(Boolean).pop() || cwd || '项目';
    }
    ...
    function applyHiddenProjects(hidden) {
        ...
        if (hidden.includes(state.projectAliases.get(state.cwd) || state.cwd) && !state.session) {
            ...
            elements.projectName.textContent = '选择项目';
            elements.projectPath.textContent = '';
            elements.projectInput.value = '';
            clearSessionView();
            setConnection('idle', '选择项目开始工作');
        }
    }
    ...
    async function removeEmptyProject(cwd) {
        ...
        try {
            ...
            if (!Array.isArray(data.hiddenProjects) || !data.hiddenProjects.includes(resolved)) {
                throw new Error('服务端未确认项目已移出，请刷新后重试');
            }
            ...
            renderProjects();
            renderSessions();
            toast('项目已从列表移除，目录和文件保留', 'success');
        } catch (error) {
            toast(error.message, 'error');
        } finally {
    ...
    const ACTIVITY_LABELS = {
        running: ['处理中', 'fa-spinner fa-spin'], tool: ['工具执行', 'fa-screwdriver-wrench'],
        compacting: ['压缩中', 'fa-compress'], retrying: ['重试中', 'fa-rotate-right'],
        idle: ['空闲', 'fa-circle-check'], error: ['失败', 'fa-circle-exclamation'],
        stopped: ['已停止', 'fa-stop'], waiting: ['等待确认', 'fa-hand'], inactive: ['未运行', 'fa-circle'], unknown: ['状态未知', 'fa-question-circle']
    };
    ...
    function renderWorkSessions() {
        ...
        const sections = [
            ['attention', '需处理', rows.filter(row => row.dataset.workStage === 'attention')],
            ['running', '处理中', rows.filter(row => row.dataset.workStage === 'running')],
            ['unknown', '状态待更新', rows.filter(row => row.dataset.workStage === 'unknown')],
            ['recent', '最近会话', recent.slice(0, recentLimit).map(item => item.row)]
        ];
        let count = 0;
        for (const [kind, label, items] of sections) {
            ...
            if (kind === 'recent') {
                ...
                toggle.textContent = state.recentSessionsExpanded ? '收起（显示 6 条）' : `展开更多（${Math.min(recent.length, 12)} 条）`;
            }
            count += items.length;
        }
        return count;
    ...
    function renderActivityBadges() {
        ...
        elements.sessionList.querySelectorAll('[data-session-id]').forEach(row => {
            ...
            badge.title = phase === 'inactive' ? '本工作台没有此会话的运行实例；不代表外部终端状态' : label;
            ...
            unreadBadge.lastChild.textContent = notice?.manual ? ' 未读' : ' 新回复';
            ...
            delayedBadge.textContent = deferred?.count ? `${deferred.count} 条待发送${deferred.attention ? ' · 需确认' : ''}` : '';
        ...
        elements.sessionList.querySelectorAll('[data-project-cwd]').forEach(group => {
            ...
            badge.textContent = !state.activityFresh ? '状态未知' : [delayedAttention && `${delayedAttention} 个延迟待确认`, waiting && `${waiting} 个待确认`, unread && `${unread} 个未读`, busy && `${busy} 个处理中`, failed && `${failed} 个失败`].filter(Boolean).join(' · ');
        ...
        empty.textContent = !state.activityFresh ? (state.activityEnabled ? '工作状态暂不可用，显示已加载会话' : '正在读取工作状态')
            : state.loadingProjects.size ? '正在读取会话' : elements.sessionSearch.value.trim() ? '没有匹配的工作会话' : '暂无需处理、处理中或最近会话';
    ...
    function renderProjects() {
        if (!state.projects.length) {
            elements.projectList.innerHTML = '<div class="pi-list-state">暂无最近项目</div>';
            return;
        }
        elements.projectList.innerHTML = orderedProjects(elements.showHiddenProjects.checked).map(project => `
            <button type="button" class="pi-project-row" data-cwd="${escapeHtml(project.cwd)}">
                ...
                <em>${state.hiddenProjects.includes(state.projectAliases.get(project.cwd) || project.cwd) ? '已移出' : project.sessionCount || 0}</em>
            </button>
        `).join('');
    }
    ...
    async function selectProject(cwd, options = {}) {
        ...
        if (state.projectIdentity) {
            ...
            if (!cwd) throw new Error('无法确认项目路径');
            state.projectAliases.set(input, cwd);
        }
        ...
        if (projectChanged) {
            ...
            clearSessionView();
            setConnection('connecting', '正在读取项目会话');
        }
        ...
        try {
            ...
            renderProjects();
            renderSessions();
            setConnection('idle', state.session ? elements.connectionText.textContent : '选择会话开始工作');
    ...
    function getSessionTitle(session) {
        if (session.ephemeral) return '临时会话';
    ...
    function threadOverflowControl(cwd, hiddenSessions, total) {
        if (hiddenSessions.length) {
            ...
            const summary = [
                flags.filter(item => item.busy).length && `${flags.filter(item => item.busy).length} 个处理中`,
                flags.filter(item => item.attention).length && `${flags.filter(item => item.attention).length} 个需关注`
            ].filter(Boolean).join(' · ');
            return `<button class="pi-thread-more" type="button" data-project-action="more-threads" title="展开该项目其余 ${hiddenSessions.length} 个线程"><i class="fa-solid fa-ellipsis" aria-hidden="true"></i><span>显示其余 ${hiddenSessions.length} 个线程</span>${summary ? `<em>${summary}</em>` : ''}</button>`;
        }
        if (state.expandedThreadLists.has(cwd) && total > THREAD_PREVIEW_LIMIT) {
            return `<button class="pi-thread-more" type="button" data-project-action="less-threads"><i class="fa-solid fa-chevron-up" aria-hidden="true"></i><span>仅显示前 ${THREAD_PREVIEW_LIMIT} 个线程</span></button>`;
        }
        return '';
    }
        ...
        return `
            <article class="pi-session-item ${session.ephemeral ? 'ephemeral' : ''} ${state.cwd === cwd && state.session?.id === session.id ? 'active' : ''}" data-session-id="${escapeHtml(session.id)}" data-cwd="${escapeHtml(cwd)}">
                <button class="pi-session-main" type="button">
                    ...
                    <span class="pi-session-title">${escapeHtml(getSessionTitle(session))}${session.ephemeral ? '<em>不保存</em>' : ''}</span>
                    <span class="pi-session-preview">${session.ephemeral ? '退出或断开后立即销毁' : escapeHtml(truncate(session.firstMessage === '(no messages)' ? '' : session.firstMessage, 62) || '空会话')}</span>
                    <span class="pi-session-deferred" hidden></span>
                    <span class="pi-session-unread" hidden><i class="fa-solid fa-circle" aria-hidden="true"></i> 新回复</span>
                    <span class="pi-session-foot"><span class="pi-session-activity"></span><span>${session.ephemeral ? '临时' : `${formatDate(session.modified)} · ${session.messageCount} 条`}</span></span>
                </button>
                <div class="pi-session-actions">
                    <button type="button" data-action="menu" title="线程操作" aria-label="线程操作" aria-haspopup="menu" aria-expanded="false"><i class="fa-solid fa-ellipsis"></i></button>
                </div>
            </article>
        `;
    }
    ...
    function renderSessions() {
        ...
        const groups = orderedProjects().map(project => {
            ...
            if (loading && !source.length) {
                threadContent = '<div class="pi-project-loading"><span class="pi-spinner"></span>正在读取线程</div>';
                ...
                threadContent = '<div class="pi-project-empty">暂无线程</div>';
            }
            ...
            return `
                <section class="pi-project-group ${state.pinnedProjects.includes(cwd) ? 'pinned' : ''} ${isCurrent ? 'current' : ''} ${expanded ? 'expanded' : ''}" data-project-cwd="${escapeHtml(cwd)}">
                    <div class="pi-project-group-heading">
                        <button class="pi-project-disclosure" type="button" data-project-action="toggle" title="${expanded ? '折叠线程' : '展开线程'}" aria-label="${expanded ? '折叠' : '展开'} ${escapeHtml(project.name || getProjectName(cwd))} 的线程" aria-expanded="${expanded}">
                            <i class="fa-solid fa-chevron-right"></i>
                        </button>
                        <button class="pi-project-group-main" type="button" data-project-action="toggle" aria-expanded="${expanded}" title="${escapeHtml(cwd)}（点击${expanded ? '折叠' : '展开'}线程，点击线程打开项目）">
                        ...
                        <button class="pi-project-pin" type="button" data-project-action="pin" aria-pressed="${state.pinnedProjects.includes(cwd)}" title="${state.pinnedProjects.includes(cwd) ? '已置顶，点击取消' : '置顶项目'}" aria-label="${state.pinnedProjects.includes(cwd) ? '取消置顶' : '置顶项目'} ${escapeHtml(project.name || getProjectName(cwd))}" ${state.pinRequests.has(cwd) ? 'disabled' : ''}><i class="fa-solid fa-thumbtack" aria-hidden="true"></i></button>
                        <button class="pi-project-new" type="button" data-project-action="menu" title="项目操作" aria-label="项目操作" aria-haspopup="menu" aria-expanded="false"><i class="fa-solid fa-ellipsis"></i></button>
                    </div>
        ...
        elements.sessionCount.textContent = `${orderedProjects().length} 个项目 · ${state.sessions.length} 个当前线程`;
        elements.sessionList.innerHTML = groups || (state.sessionFilter === 'work' ? '' : `<div class="pi-list-state">${query ? '没有匹配的项目或线程' : '还没有 Pi 项目'}</div>`);
        renderActivityBadges();
        elements.sessionList.scrollTop = scrollTop;
    ...
    async function markThreadUnread(session, cwd) {
        ...
        try {
            ...
            if (!notice?.completionId || notice.cwd !== cwd || notice.sessionId !== session.id) throw new Error('未读标记未得到确认');
    ...
    async function deleteOneSession(session = state.session, cwd = session?.cwd || state.cwd) {
        if (!session || session.ephemeral || !cwd) return;
        if (!window.confirm(`删除会话“${getSessionTitle(session)}”？\n会先尝试将会话文件移入回收站；回收站不可用时将永久删除。`)) return;
        try {
            ...
            renderProjects();
            renderSessions();
            toast(result?.trashed === true ? '会话已移入回收站' : result?.trashed === false ? '会话已永久删除' : '会话已删除', 'success');
        } catch (error) {
            toast(error.message, 'error');
        }
    }
    ...
    async function openSession(session) {
        ...
        elements.transcript.innerHTML = `<div class="pi-transcript-loading"><span class="pi-spinner"></span><span>${session.ephemeral ? '正在启动临时 Pi runtime' : '正在加载 Pi session'}</span></div>`;
        setConnection('connecting', '正在启动 Pi runtime');
        setAgentState('connecting', '正在启动', getSessionTitle(session));
        ...
        return new Promise((resolve, reject) => {
            ...
            socket.addEventListener('open', async () => {
                ...
                try {
                    ...
                    setConnection(state.shellBusy || state.compacting || state.streaming ? 'working' : 'connected', state.shellBusy ? 'Shell 正在执行' : state.compacting ? '正在压缩上下文' : state.streaming ? 'Pi 正在执行' : 'Pi Agent 已连接');
                    resolve();
                } catch (error) {
                    if (generation === state.socketGeneration && state.socket === socket) {
                        if (['STARTUP_UI_UNSUPPORTED', 'RPC_STARTUP_FAILED'].includes(error.errorCode)) {
                            ...
                            for (const [label, action] of [['重试连接', () => void connectSocket(session).catch(() => {})], ['管理扩展', () => window.dispatchEvent(new CustomEvent('workspace:open-settings', { detail: { tab: 'packages' } }))]]) {
    ...
    function requestRpc(type, payload = {}, timeoutMs = 45000) {
        if (!state.socket || state.socket.readyState !== WebSocket.OPEN) {
            return Promise.reject(new Error('Pi Agent 尚未连接'));
        }
        const id = `browser-${state.nextRequestId++}`;
        return new Promise((resolve, reject) => {
            const timer = setTimeout(() => {
                state.requests.delete(id);
                reject(Object.assign(new Error(`${type} 请求超时`), { deliveryUnknown: true }));
            }, timeoutMs);
    ...
    function handleSocketRecord(raw) {
        ...
        if (record.type === 'response' && record.id && state.requests.has(record.id)) {
            ...
            if (record.success) pending.resolve(record.data);
                ...
                const message = record.error || `${record.command || '请求'}失败`;
                pending.reject(Object.assign(new Error(message), { errorCode: record.errorCode, deliveryUnknown: /timed out|timeout|exited|closed|EPIPE|ECONNRESET|not running/i.test(message) }));
            }
            return;
        }
        if (state.bootstrapEvents) {
            ...
            if (state.bootstrapBytes > 8 * 1024 * 1024 || state.bootstrapEvents.length >= 10000) {
                state.intentionalClose = true; setConnection('error', '初始化期间事件过多，请重新打开线程');
                state.socket.close(4008, '初始化期间事件过多，请重新打开线程'); return;
            }
    ...
    function restoreLive(live) {
        ...
        if (live.truncated) appendEventNotice('info', '运行现场超过恢复预览限额，部分内容未显示；最终消息以 Pi 完成记录为准。');
    }
    ...
    function applySnapshot(snapshot) {
        ...
        setAgentState(
            ...
            state.shellBusy ? 'Shell 执行中' : state.compacting ? '压缩中' : state.streaming ? '执行中' : '空闲',
    ...
    function syncModelGuidance() {
        ...
        elements.modelGuidanceText.textContent = state.modelRefreshOp ? (state.modelRefreshOp.kind === 'selection' ? '正在切换本会话的模型…' : '正在更新当前会话的可用模型…') : state.modelRefreshError
            || (state.modelAuthError ? '当前模型的认证不可用。请重新登录供应商，或选择其他已接入模型。'
                : state.models.length ? '请选择本会话使用的模型。设置中的默认模型不会自动替换已打开会话的模型。'
    ...
    async function refreshSessionModels() {
        ...
        try {
            ...
            if (!state.modelCatalogSupported && !state.models.length) state.modelRefreshError = '尚无可用模型。请先完成供应商设置；旧后端需在空闲时退出并重开线程后读取新配置。';
            renderModels();
        } catch (error) {
    ...
    function renderModels() {
        ...
        const placeholder = available ? '' : `<option value="" disabled selected>${state.models.length ? '请选择已接入的模型' : '尚无可用模型'}</option>`;
    ...
    function renderThinkingLevels() {
        const labels = { off: '关闭', minimal: '极简', low: '低', medium: '中', high: '高', xhigh: '极高', max: '最大' };
    ...
    const messageTimeFormat = new Intl.DateTimeFormat('zh-CN', {
    ...
    function createMessageContentElement(message, showReplyActions) {
        ...
        const title = role === 'user' ? '你' : role === 'assistant' ? 'Pi' : '系统';
        ...
        if (role === 'user') {
            if (typeof message.timestamp === 'number' || typeof message.timestamp === 'string') {
                ...
                if (Number.isFinite(sentAt.getTime())) {
                    ...
                    time.title = sentAt.toLocaleString('zh-CN', { hour12: false });
        ...
        if (message.stopReason === 'aborted') {
            ...
            notice.textContent = '本次回复已达到输出上限，可发送消息要求继续。'; body.appendChild(notice);
        } else if (message.errorMessage || message.stopReason === 'error') {
            const error = document.createElement('div');
            error.className = 'pi-inline-error';
            error.textContent = message.errorMessage || '回复失败';
        ...
        if (responseText && showReplyActions) {
            ...
            if (window.PiReplyTts.enabled) {
                ...
                speech.title = '朗读回复（TTS）'; speech.setAttribute('aria-label', speech.title);
    ...
    function createThinkingBlock(text = '') {
        ...
        details.innerHTML = '<summary><i class="fa-solid fa-brain"></i><span>思考过程</span><i class="fa-solid fa-chevron-right"></i></summary><div class="pi-thinking-content"></div>';
    ...
    function createToolCallBlock(toolCall) {
        ...
        row.innerHTML = `
            <summary>
                ...
                <span class="pi-tool-status">等待结果</span>
                <i class="fa-solid fa-chevron-right"></i>
            </summary>
    ...
    function createToolResultElement(message) {
        ...
        row.querySelector('.pi-tool-status').textContent = message.isError ? '失败' : '完成';
    ...
    function createNoticeElement(type, text) {
        ...
        notice.innerHTML = `<i class="fa-solid ${type === 'compactionSummary' ? 'fa-compress' : 'fa-code-branch'}"></i><div><strong>${type === 'compactionSummary' ? '上下文已压缩' : '分支摘要'}</strong><p></p></div>`;
    ...
    function renderEmptySession() {
        elements.transcript.innerHTML = `
            <div class="pi-empty-state session-ready">
                ...
                <p>${state.session ? escapeHtml(state.cwd) : '选择项目会话后开始工作'}</p>
            </div>
        `;
        transcriptView.refresh();
    }
    ...
    function clearSessionView() {
        ...
        state.stats = null;
        state.streaming = false;
        elements.modelSelect.innerHTML = '<option>等待会话</option>';
        elements.modelSelect.disabled = true;
        elements.thinkingSelect.innerHTML = '<option>关闭</option>';
        elements.thinkingSelect.disabled = true;
        elements.input.disabled = true;
        elements.sendButton.disabled = true;
        elements.compactButton.disabled = true;
        elements.currentThreadMenu.disabled = true;
        elements.composerStatus.textContent = '未打开会话';
        updateStats(null);
        updateSessionMeta();
        setAgentState('idle', '未连接', '等待会话');
    }
    ...
    function handlePiEvent(event) {
        ...
        switch (event.type) {
            ...
            case 'agent_start':
                setStreaming(true);
                setConnection('working', 'Pi 正在执行');
                setAgentState('working', '执行中', state.model?.id || 'Pi Agent');
                break;
            case 'agent_settled':
                historyView.changed();
                setStreaming(false);
                setConnection('connected', 'Pi Agent 已连接');
                setAgentState('connected', '空闲', state.model?.id || 'Pi Agent');
                reconcileSession();
                break;
            case 'message_start':
            ...
            case 'extension_error':
                toast(event.error || '扩展命令执行失败', 'error');
                break;
            ...
            case 'compaction_start':
                ...
                setStreaming(state.streaming);
                setConnection('working', '正在压缩上下文');
                setAgentState('working', '压缩中', state.model?.id || 'Pi Agent');
            ...
            case 'compaction_end':
                ...
                renderCompactionStatus();
                setStreaming(state.streaming);
                setConnection(state.streaming ? 'working' : 'connected', state.streaming ? 'Pi 正在执行' : 'Pi Agent 已连接');
                setAgentState(state.streaming ? 'working' : 'connected', state.streaming ? '执行中' : '空闲', state.model?.id || 'Pi Agent');
                if (state.compaction.status === 'error' && event.errorMessage) toast(`压缩失败：${event.errorMessage}`, 'error', 8000);
            ...
            case 'summarization_retry_scheduled':
                ...
                renderCompactionStatus();
                setConnection('working', '压缩摘要等待重试');
                break;
            case 'summarization_retry_attempt_start':
                if (!state.compacting) break;
                state.compaction = { ...state.compaction, status: 'running' };
                renderCompactionStatus();
                setConnection('working', '正在重试压缩摘要');
                break;
            case 'summarization_retry_finished':
                break;
            case 'auto_retry_start':
                appendEventNotice('retry', `请求失败，${Math.ceil((event.delayMs || 0) / 1000)} 秒后进行第 ${event.attempt} 次重试`);
                break;
            case 'auto_retry_end':
                appendEventNotice(event.success ? 'success' : 'error', event.success ? '自动重试成功' : event.finalError || '自动重试失败');
                break;
            ...
            case 'gateway_ui_resolved':
                ...
                break;
            case 'gateway_error':
                toast(event.error || 'Pi Agent 发生错误', 'error', 7000);
                break;
        }
    }
    ...
    function startLiveMessage(message) {
        ...
        article.innerHTML = '<header><strong>Pi</strong><span class="pi-stream-label">生成中</span></header><div class="pi-message-body"></div>';
    ...
    function updateToolExecution(event, status) {
        ...
        row.querySelector('.pi-tool-status').textContent = status === 'running' ? '执行中' : status === 'error' ? '失败' : '完成';
        if (event.args) {
    ...
    function renderPendingUi() {
        ...
        $('pi-pending-ui-count').textContent = `${state.pendingUi.size} 项等待确认`;
        renderActivityBadges();
        setStreaming(state.streaming);
    }
    ...
    function openPendingRequest() {
        ...
        elements.requestTitle.textContent = event.title || '等待确认';
        ...
        if (event.method !== 'confirm') {
            ...
            field.name = 'value';
            field.setAttribute('aria-label', event.title || '输入内容');
    ...
    async function reconcileSession() {
        ...
        try {
            ...
            if (state.connected) toast(`会话同步失败：${error.message}`, 'error');
        }
    }
    ...
    function updateStats(stats) {
        ...
        const fraction = hasUsage ? `${formatTokens(usage.tokens)} / ${capacity}` : capacity ? `-- / ${capacity}` : '上下文';
        elements.contextPercent.textContent = hasUsage ? `${Math.round(percent)}%` : '--';
        elements.contextTokens.textContent = fraction;
        elements.contextButton.title = hasUsage ? '会话统计' : capacity ? '压缩后用量待更新，等待下一次模型回复' : '上下文用量暂无统计';
    ...
    function updateSessionMeta(runtime) {
        ...
        elements.metaFile.textContent = ephemeral ? '不保存（pi --no-session）' : state.session?.path || runtime?.sessionFile || '--';
        elements.metaFile.title = ephemeral ? '临时 runtime 不创建 session 文件' : state.session?.path || runtime?.sessionFile || '';
    }
    ...
    function setStreaming(streaming) {
        ...
        if (state.connected && state.shellBusy) { setConnection('working', 'Shell 正在执行'); setAgentState('working', 'Shell 执行中', state.model?.id || 'Pi Agent'); }
        ...
        const stopAction = nativeControls.enabled ? compacting ? '取消压缩并取回待发文字' : '停止并取回待发文字' : '停止当前任务';
        elements.stopButton.title = state.connected ? `${stopAction}（${getSessionTitle(state.session)}）` : '等待当前会话连接';
        ...
        elements.sendButton.title = streaming ? elements.deliveryMode.value === 'follow_up' ? '发送后续消息' : '发送引导消息' : '发送';
        ...
        elements.composerStatus.textContent = !state.connected ? '未打开会话' : state.shellBusy ? 'Shell 正在执行，可继续编辑草稿；停止请使用命令卡片' : state.treeBusy ? '正在切换对话位置，请在历史页查看进度或取消' : state.resourceRequested ? '正在重新加载原生资源' : stopping ? '正在停止 / 取回，请等待确认' : compacting ? '上下文压缩中' : streaming ? 'Agent 运行中：引导在本轮工具后送达，后续在任务完成后送达' : 'Pi Agent 已就绪';
        historyView.sync();
        workflows.update();
        sideChat.updateParent();
        autoResizeInput();
    ...
    async function takeRuntimeQueue(stop = true) {
        ...
        try {
            ...
            if (generation !== state.socketGeneration) return;
            toast(stop ? '已停止；取回文字可在运行队列中查看' : '已取回队列文字，请核对后追加到草稿', 'success');
        } catch (error) {
            if (generation !== state.socketGeneration) return;
            toast(`${error.message}；请核对运行状态和取回列表，未自动重试`, 'error', 8000);
            await reconcileSession();
        } finally {
    ...
    async function copyLastResponse() {
        try {
            ...
            toast('已复制最近一条 Agent 回复', 'success');
        } catch (error) {
            toast(error.message, 'error');
        }
    }
    ...
    async function quitRuntime() {
        if (!state.connected) return toast('当前没有运行中的 Pi runtime', 'error');
        ...
        try {
            ...
            elements.thinkingSelect.disabled = true;
            elements.currentThreadMenu.disabled = true;
            elements.composerStatus.textContent = 'Pi runtime 已退出';
            setConnection('idle', 'Pi runtime 已退出');
            setAgentState('idle', '已退出', wasEphemeral ? '临时会话已销毁' : '重新打开会话可继续');
            renderSessions();
            updateSessionMeta();
            toast(wasEphemeral ? '临时会话已销毁' : 'Pi runtime 已退出，会话记录已保留', 'success');
            return true;
        } catch (error) {
            toast(error.message, 'error');
            return false;
    ...
    async function reloadResources() {
        if (!state.connected || state.shellBusy || state.streaming || state.compacting || state.compactRequested || state.controlRequested || state.resourceRequested || state.pendingUi.size) throw new Error('请等待当前任务与确认结束后重新加载资源');
        const generation = state.socketGeneration;
        ...
        try {
            ...
            if (generation === state.socketGeneration) toast('原生资源已重新加载，命令目录已更新', 'success');
        } finally {
    ...
    async function executeLocalCommand(name, args, draft) {
        if (name === 'import' || name === 'export') {
            if (!transfer.enabled) throw new Error('当前后端尚未启用会话导入与导出');
            if (args) throw new Error(`网页 /${name} 不接收文件路径参数，请直接输入 /${name} 后在窗口中选择`);
            if (name === 'export') {
                if (!state.session || state.session.ephemeral) throw new Error('请先打开一个已保存的线程再导出');
                return transfer.openExport(state.session, state.cwd);
            }
            return transfer.openImport(state.cwd);
        }
        if (name === 'tree') {
            if (args) throw new Error('请输入 /tree 后在会话树中选择位置');
            if (!historyView.tree.enabled) throw new Error('当前后端尚未启用会话树');
        ...
        if (name === 'copy') { await copyTextToClipboard(state.lastAssistantText); toast('已复制最近一条 Agent 回复', 'success'); return true; }
        ...
        if (name === 'trust') {
            if (!state.nativeSettings) throw new Error('当前后端尚未启用项目信任设置');
            if (args) throw new Error('请在项目信任窗口选择操作');
        ...
        if (name === 'hotkeys') { toast('输入 / 搜索命令、Skill 或模板，@ 搜索项目文件；↑↓ 选择，Tab/Enter 填入，Esc 关闭；加号内可添加附件或延迟发送，Shift+Enter 换行。', 'info', 12000); return true; }
        if (!state.connected) throw new Error('请先打开会话');
        if (name === 'fork' || name === 'clone') {
            if (!workflows.enabled) throw new Error('当前后端尚未支持会话分叉');
        ...
        if (name === 'name') {
            const value = args || window.prompt('会话名称', getSessionTitle(state.session));
            if (value === null || !value.trim()) return false;
            if (state.session.ephemeral) throw new Error('临时会话不保存名称');
        ...
        if (name === 'model' || name === 'thinking') {
            if (state.shellBusy || state.streaming || state.compacting || state.resourceRequested) throw new Error('请等待会话空闲后切换模型或思考等级');
            if (!args) {
                if (name === 'model' && !state.models.length) { syncModelGuidance(); elements.modelSetup.focus(); toast('请先接入聊天模型；命令草稿已保留', 'info'); return false; }
            ...
            if (name === 'model') {
                ...
                if (models.length !== 1) throw new Error('请使用目录中的完整 provider/model，或不带参数打开选择器');
                ...
                if (!state.thinkingLevels.includes(args)) throw new Error('该模型不支持此思考等级，请从当前下拉框选择');
        ...
        throw new Error('此终端命令尚未接入网页，请使用对应终端功能');
    }

    async function sendMessage() {
        const key = state.composerSessionKey;
        if (state.shellBusy && elements.input.value.trim() !== '/quit') return toast('Shell 正在执行，请等待命令结束后发送；/quit 可退出运行实例', 'info');
        if (nativeControls.value?.stopping || state.controlRequested) return toast('正在停止或取回队列，请等待状态确认', 'info');
        if (state.submittingDrafts.has(key)) return;
        if (state.attachmentReads) return toast('附件正在读取，请稍候', 'info');
        if (state.treeBusy) return toast('正在切换对话位置，请等待结果；可在历史页取消导航', 'info');
        if (state.modelRefreshOp) return toast('正在更新模型目录，请稍候；草稿已保留', 'info');
        if (state.resourceRequested) return toast('正在重新加载原生资源，请稍候', 'info');
        ...
        if (slash && !selectedCommand) return toast('未找到此命令，请输入 / 查看可用命令；刚保存的模板需重新加载资源', 'info');
        ...
        if (selectedCommand && ['web', 'builtin'].includes(selectedCommand.source)) {
            if (state.attachmentFiles.length) return toast('此命令不接收上传附件，请先处理附件', 'info');
            ...
            catch (error) { if (key === state.composerSessionKey) toast(`${error.message}；命令草稿已保留`, 'error'); }
            finally { state.submittingDrafts.delete(key); autoResizeInput(); }
            return;
        }
        if (selectedCommand?.source === 'extension' && state.attachmentFiles.length) return toast('扩展命令不接收上传附件，请先处理附件', 'info');
        if (!text && !state.attachmentFiles.length) return;
        if (!state.connected) return toast('请先打开会话', 'error');
        if (state.modelRefreshOp) return toast('正在更新模型目录，请稍候；草稿已保留', 'info');
        if (selectedCommand?.source !== 'extension' && !usableSessionModel()) {
            syncModelGuidance();
            (state.models.length ? elements.modelChoose : elements.modelSetup).focus();
            return toast(state.models.length ? '请先选择已接入的模型；草稿已保留' : '请先在供应商与模型设置中完成接入；草稿已保留', 'info');
        }
        if (state.compacting || state.compactRequested) return toast('上下文正在压缩，请等待完成', 'info');

        try { attachments.validateDraft(elements.input.value, state.attachmentFiles); }
        catch (error) { return toast(error.message, 'error'); }
        if (state.uncertainDrafts.has(key) && !window.confirm('上次发送结果不确定。请先核对会话，重复发送可能重复执行任务。确认仍要发送此草稿？')) return;
        ...
        if (images.length && !state.model?.input?.includes('image')) {
            return toast('当前模型不支持图片输入，请先切换多模态模型', 'error', 6500);
        }
        ...
        composer.hide();
        const optimistic = selectedCommand ? null : appendOptimisticUser(text || textAttachments.map(file => `附件：${file.name}`).join(', '), images, message);
        try {
            ...
            const notice = error.deliveryUnknown ? '发送结果不确定，草稿已保留；请先核对会话，勿直接重复发送' : `${error.message}；草稿已保留`;
    ...
    async function sendShell(input) {
        if (!state.userShell) return toast('当前后端尚未启用 Shell，命令草稿已保留', 'info');
        if (!state.connected || state.streaming || state.shellBusy || state.compacting || state.compactRequested || state.pendingUi.size) return toast('请等待当前会话空闲后执行 Shell', 'info');
        if (state.attachmentFiles.length) return toast('Shell 命令不接收附件，请先移除或发送附件', 'info');
        if (!input.command || input.command.length > 32768) return toast('请输入非空命令，最多 32768 字符', 'info');
        ...
        if (state.uncertainDrafts.has(draft.key) && !window.confirm('上次提交结果不确定。请先核对原生记录和命令状态；重复执行可能再次修改文件。仍要执行？')) return;
        ...
        try {
            ...
            if (draft.generation === state.socketGeneration) {
                toast(`${error.message}；草稿已保留${error.deliveryUnknown ? '，请先核对命令状态，勿直接重复执行' : ''}`, 'error', 8000);
                void reconcileSession();
            }
    ...
    function autoResizeInput() {
        ...
        const shellInput = window.PiShell.parse(elements.input.value);
        shell.sync();
        elements.sendButton.title = shellInput ? '执行 Shell 命令' : state.streaming ? elements.deliveryMode.value === 'follow_up' ? '发送后续消息' : '发送引导消息' : '发送';
        elements.sendButton.setAttribute('aria-label', elements.sendButton.title);
        ...
        status.textContent = pending ? '正在读取附件…' : submitting ? '正在提交消息…' : state.uncertainDrafts.has(state.composerSessionKey) ? '上次发送结果不确定，草稿已保留，请先核对会话' : '';
    ...
    function handleFiles(fileList) {
        ...
        if (!state.connected) return toast('请先打开会话', 'info');
        if (state.submittingDrafts.has(state.composerSessionKey)) return toast('请等待消息投递完成', 'info');
        ...
        state.attachmentQueue = state.attachmentQueue.then(async () => {
            ...
            for (const file of files) {
                ...
                try {
                    if (state.attachmentFiles.length >= attachments.limits.files) throw new Error('一次最多添加 8 个附件');
                    if (attachments.classify(file).kind === 'image' && state.attachmentFiles.filter(item => item.kind === 'image').length >= attachments.limits.images) throw new Error('一次最多添加 6 张图片');
    ...
    function renderAttachments() {
        ...
        elements.attachments.innerHTML = state.attachmentFiles.map(file => `
            <div class="pi-attachment-chip" data-attachment-id="${file.id}">
                <button class="pi-attachment-open" type="button" data-preview-attachment title="预览附件" aria-label="预览 ${escapeHtml(file.name)}">${file.kind === 'image' ? `<img src="${file.preview}" alt="">` : '<span class="pi-file-mark"><i class="fa-solid fa-file-code"></i></span>'}</button>
                <span title="${escapeHtml(file.name)}"><strong>${escapeHtml(file.name)}</strong><small>${file.size == null ? '图片' : `${Math.ceil(file.size / 1024)} KB`}</small></span>
                <button type="button" data-remove-attachment title="移除附件" aria-label="移除附件"><i class="fa-solid fa-xmark"></i></button>
            </div>
        `).join('');
    }
    ...
    async function changeModel() {
        ...
        try {
            ...
            updateRuntimeState(runtime);
            updateStats(stats);
            toast(`已切换到 ${state.model.name || state.model.id}`, 'success');
        } catch (error) {
            if (generation !== state.socketGeneration) return;
            toast(error.message, 'error');
            renderModels();
    ...
    async function changeThinking() {
        ...
        try {
            ...
            toast(`思考等级：${elements.thinkingSelect.selectedOptions[0].textContent}`, 'success');
        } catch (error) {
            toast(error.message, 'error');
        } finally {
    ...
    async function runCompact(customInstructions = '', draft = null) {
        if (!state.connected || state.shellBusy || state.compacting || state.compactRequested || state.streaming || state.pendingUi.size) {
            toast('请等待当前任务完成，或先停止任务再压缩', 'info');
            return false;
        }
        const generation = state.socketGeneration;
        ...
        try {
            ...
            if (generation === state.socketGeneration && state.connected && !reported) {
                toast(/timed out|超时/i.test(error.message) ? '压缩等待超时，正在重新同步运行状态；后台操作可能仍在继续' : error.message, 'error', 8000);
            }
            return false;
        } finally {
            if (generation === state.socketGeneration) {
    ...
    function renderCompactionStatus() {
        ...
        let text = '';
        const reason = compact?.reason === 'manual' ? '手动压缩' : compact?.reason === 'overflow' ? '溢出恢复压缩' : '自动压缩';
        if (status === 'running') text = `${reason}中`;
        else if (status === 'retrying') text = `${reason}：${Math.ceil((compact.delayMs || 0) / 1000)} 秒后重试 ${compact.attempt}/${compact.maxAttempts}`;
        else if (status === 'cancelled') text = `${reason}已取消`;
        else if (status === 'unchanged') text = '暂无新增内容需要压缩';
        else if (status === 'error') text = `${reason}失败：${compact.errorMessage || '未返回压缩结果'}`;
        else if (status === 'success') {
            text = `${reason}完成`;
            if (Number.isFinite(compact.tokensBefore)) text += `，压缩前 ${formatTokens(compact.tokensBefore)}`;
            if (Number.isFinite(compact.estimatedTokensAfter)) text += `，压缩后约 ${formatTokens(compact.estimatedTokensAfter)}`;
        }
        if (state.stats?.contextUsage?.tokens === null) text += `${text ? '。' : ''}当前用量待下一次回复更新`;
    ...
    function openProjectDialog() {
        ...
        $('pi-project-scope').textContent = state.roots.includes('/') ? '选择服务器上当前用户有权限访问的目录；不是这台浏览器电脑的目录。' : `可选范围：${state.roots.join('、')}。需要其他目录时，可调整服务器的项目范围（PI_PROJECT_ROOTS）。`;
        renderProjects();
    ...
    async function browseDirectories(directory) {
        ...
        try {
            ...
            elements.directoryCurrent.textContent = data.current || '可选位置';
            elements.directoryUp.disabled = !data.current;
            elements.directoryUp.title = elements.directoryUp.ariaLabel = data.parent ? '上级目录' : '返回可选位置';
            ...
            `).join('') || '<div class="pi-list-state">没有子目录</div>';
        } catch (error) {
            if (revision !== state.directoryRequest || elements.projectDialog.classList.contains('hidden')) return;
    ...
    function historyWorkflowItems() {
        return [
            { label: '从历史问题分叉', icon: 'fa-code-branch', disabled: !workflows.usable(), run: () => workflows.run(() => workflows.openList('history')) },
            { label: '恢复旧版本', icon: 'fa-clock-rotate-left', disabled: workflows.busy(), run: () => workflows.run(() => workflows.openList('versions')) }
        ];
    }
    $('pi-history-more').addEventListener('click', event => {
        threadMenu.open(event.currentTarget, historyWorkflowItems());
        threadMenu.element.setAttribute('aria-label', '历史操作');
    });
    ...
    function showThreadMenu(target, point, current = false) {
        ...
        if (row || current) {
            ...
            if (!session) return;
            items = [
                ...(historyView.tree.enabled ? [command('会话树', 'fa-code-branch', async () => {
                ...
                ...(historyView.enabled ? [command('搜索历史与书签', 'fa-magnifying-glass', async () => {
                    if (cwd !== state.cwd) await selectProject(cwd);
                    if (state.session?.id !== session.id || !state.connected) await openSession(session);
                    historyView.open();
                }, { disabled: session.ephemeral })] : []),
                ...(transfer.enabled ? [command('导出记录', 'fa-download', () => transfer.openExport(session, cwd), { disabled: session.ephemeral })] : []),
                command('重命名', 'fa-pen', () => renameCurrent(session, cwd), { disabled: session.ephemeral }),
                ...(workflows.enabled ? [command('复制为新线程', 'fa-code-branch', async () => {
                    if (cwd !== state.cwd) await selectProject(cwd);
                    if (state.session?.id !== session.id || !state.connected) await openSession(session);
                    await workflows.openClone();
                }, { disabled: session.ephemeral }), command('待发送消息', 'fa-clock', async () => {
                    if (cwd !== state.cwd) await selectProject(cwd);
                    if (state.session?.id !== session.id || !state.connected) await openSession(session);
                    await workflows.openList('deferred');
                }, { disabled: session.ephemeral })] : []),
                ...(current && workflows.enabled && !historyView.enabled ? historyWorkflowItems() : []),
                ...(state.manualUnread ? [command('标记为未读', 'fa-envelope', () => markThreadUnread(session, cwd),
                    { disabled: session.ephemeral || state.unreadRequests.has(activityKey(cwd, session.id)) })] : []),
                { label: '复制', icon: 'fa-copy', children: [
                    command('线程名称', 'fa-font', () => copyTextToClipboard(getSessionTitle(session))),
                    command('会话 ID', 'fa-fingerprint', () => copyTextToClipboard(session.id))
                ] },
                command('删除线程', 'fa-trash', () => deleteOneSession(session, cwd), { disabled: session.ephemeral, danger: true })
            ];
        } else {
            items = [
                ...(transfer.enabled ? [command('导入 Pi 会话', 'fa-file-import', () => transfer.openImport(cwd))] : []),
                command('新建线程', 'fa-plus', async () => { if (cwd !== state.cwd) await selectProject(cwd); await createSession(); }),
                command(state.pinnedProjects.includes(cwd) ? '取消置顶' : '置顶项目', 'fa-thumbtack', () => toggleProjectPin(cwd), { disabled: state.pinRequests.has(cwd) }),
                ...(state.nativeSettings ? [command('项目信任', 'fa-shield-halved', () => nativeContext.openTrust(cwd))] : []),
                command('复制项目路径', 'fa-copy', () => copyTextToClipboard(cwd)),
                command('刷新线程', 'fa-rotate', () => loadSessions(cwd)),
                command('从列表移除', 'fa-folder-minus', () => removeEmptyProject(cwd), {
    ...
    for (const type of ['dragover', 'drop']) document.addEventListener(type, event => {
        ...
        event.preventDefault();
        if (type === 'drop') toast('请将文件拖入消息输入区域', 'info');
    });
    ...
    window.PiNativeRuntime = {
        ...
        resources: async () => {
            if (!state.nativeResources || !state.connected) throw new Error('当前会话不支持资源查看');
            const generation = state.socketGeneration;
            const result = await requestRpc('get_native_resources');
            if (generation !== state.socketGeneration) throw new Error('线程已切换，请重新读取资源');
            return result;
        },
    ...
    window.addEventListener('workspace:access-locked', () => {
        state.token = ''; sideChat.disposeAll(); disconnectSocket(true); setConnection('idle', '请登录工作台');
    });
