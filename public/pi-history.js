    ...
    class PiHistoryView {
        constructor(host) {
            ...
            $('pi-history-copy').addEventListener('click', async () => {
                try { if (this.selected?.text) { await host.copy(this.selected.text); host.toast(translateUi("已复制当前预览正文"), 'success'); } } catch (e) { host.toast(e.message, 'error'); }
            });
        }
        ...
        sync() {
            ...
            this.controls();
            if (!this.available()) this.status.textContent = c.session?.ephemeral ? translateUi("临时会话不提供持久历史和书签") : translateUi("连接当前线程后可查看历史");
        ...
        async search(offset = 0) {
            ...
            this.searching = true; this.loadedGeneration = generation; this.status.textContent = translateUi("正在检索原生历史…"); this.controls();
            try {
                ...
                this.renderResults();
                if (!this.selected) this.status.textContent = data.total ? translateUi("找到 {0} 条 · 显示 {1}–{2}", data.total, data.offset + 1, data.offset + data.results.length) : translateUi("没有匹配的记录");
        ...
        renderResults() {
            ...
            for (const row of this.rows) {
                ...
                const context = [];
                if (row.hasTools && row.kind !== 'tool' && row.view === 'record') context.push(translateUi("含工具调用"));
                if (!row.inCurrentBranch) context.push(translateUi("其他分支"));
                if (context.length) button.append(node('small', context.join(' · ')));
                if (row.label) button.append(node('strong', `★ ${row.label}`));
                const snippet = node('span'); highlight(snippet, row.snippet || (row.images ? translateUi("图片附件 {0} 张", row.images) : translateUi("无可搜索正文")), this.query.value.trim()); button.append(snippet);
        ...
        leavePreview() {
            ...
            return window.confirm(translateUi("放弃尚未保存的书签名称修改？"));
        }
        async openEntry(entryId, offset, view) {
            ...
            this.status.textContent = translateUi("正在读取历史正文…");
            try {
                ...
                if (!data.inCurrentBranch) $('pi-history-preview-title').append(node('small', translateUi("其他分支")));
                ...
                } else highlight(text, data.text || (data.images ? translateUi("本条含 {0} 张图片，历史预览仅显示文字。", data.images) : translateUi("无文字正文")), this.query.value.trim());
                $('pi-history-record-toggle').hidden = !this.bodyEnabled || !data.hasTools || data.kind === 'tool';
                $('pi-history-record-toggle').textContent = data.view === 'record' ? translateUi("只看回复正文") : translateUi("查看工具参数");
                const paged = Boolean(data.offset || data.hasMore);
                $('pi-history-text-page').textContent = paged ? translateUi("{0}–{1} / {2} 字符{3}", data.offset + (data.totalCharacters ? 1 : 0), data.nextOffset, data.totalCharacters, data.images ? translateUi(" · 图片 {0} 张", data.images) : '') : data.images ? translateUi("含 {0} 张图片", data.images) : '';
                ...
                $('pi-history-copy').textContent = data.offset || data.hasMore ? translateUi("复制本页") : translateUi("复制正文");
                $('pi-history-bookmark-state').textContent = !data.canBookmark ? translateUi("首条回复保存后可添加书签") : data.labelTruncated ? translateUi("已有标签较长；如需修改，请输入新的书签名") : translateUi("书签保存在 Pi 原生会话中");
        ...
        async saveBookmark(remove) {
            ...
            if (!remove && !label) return this.host.toast(translateUi("请输入书签名称；清除请使用“移除书签”"), 'info');
            if ([...label].length > 80) return this.host.toast(translateUi("书签名称最多 80 字"), 'info');
            const { key, generation } = this.context(); this.saving = true; this.controls();
            $('pi-history-bookmark-state').textContent = translateUi("正在保存书签…");
            try {
                ...
                $('pi-history-bookmark-state').textContent = remove ? translateUi("书签已移除，原记录保留") : translateUi("书签已保存"); this.search();
            } catch (e) {
                if (this.current(key, generation) && this.selected?.entryId === selected.entryId) $('pi-history-bookmark-state').textContent = translateUi("{0}。未自动重试；可重新读取记录核对。", e.message);
            } finally { if (this.current(key, generation)) { this.saving = false; this.controls(); } }
        }
        changed(entryId) {
            this.tree.changed();
            $('pi-history-changed').textContent = translateUi("对话或书签有更新，可刷新列表查看。");
            $('pi-history-changed').hidden = false;
            if (this.selected?.entryId === entryId && !this.saving) $('pi-history-bookmark-state').textContent = translateUi("其他操作更新了此书签，请重新读取后再修改；当前编辑已保留");
        }
    }
