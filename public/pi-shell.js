(() => {
    ...
    const labels = { starting: '正在准备', running: '执行中', stopping: '正在停止', uncertain: '结果不确定', completed: '完成', failed: '失败', cancelled: '已停止', rejected: '未执行' };
    class PiShell {
        constructor(options) {
        ...
        sync() {
            ...
            this.hint.textContent = input ? `${ctx.enabled ? 'Shell' : '当前后端尚未启用 Shell'} · ${input.excludeFromContext ? '输出不加入模型上下文，仍保留原生记录' : '输出加入模型上下文，不自动发起回答'} · 默认在服务器当前项目执行` : `${pending} 条 Shell 记录可供下次提问使用，不会自动发起回答`;
        ...
        mount() {
            ...
            if (!this.card || this.card.dataset.executionId !== job.id) {
                ...
                header.append(node('strong', '手动执行 Shell'), node('span', '', 'pi-shell-state'));
                const stop = node('button', '停止命令'); stop.type = 'button'; stop.addEventListener('click', () => this.abort()); header.append(stop);
            ...
            this.card.querySelector('.pi-shell-state').textContent = `${labels[job.status] || job.status}${job.finishedAt ? ` · ${((job.finishedAt - job.startedAt) / 1000).toFixed(1)} 秒` : ''}${Number.isInteger(job.exitCode) ? ` · exit ${job.exitCode}` : ''}`;
            this.card.querySelector('.pi-shell-detail').textContent = [job.excludeFromContext ? '不加入模型上下文' : '完成后供后续提问使用', job.displayTruncated ? '显示最近部分输出' : '', job.truncated ? '原生结果已截断' : '', job.error || ''].filter(Boolean).join(' · ');
            const output = this.card.querySelector('pre');
            const following = output.scrollTop + output.clientHeight >= output.scrollHeight - 8;
            output.textContent = job.output || (this.busy ? '等待输出…' : '无输出');
        ...
        async abort() {
            ...
            catch (error) { if (epoch === this.epoch) { this.options.toast(`${error.message}；请核对命令状态，未自动重试`, 'error'); this.options.reconcile(); } }
        }
        async setMode(key, mode) {
            const epoch = this.epoch;
            if (this.modePending || !this.modes) return;
            this.modePending = true; this.modeStatus.textContent = '正在保存投递设置…'; this.sync();
            try {
                ...
                this.applyModes(value); this.modeStatus.textContent = '当前运行实例已更新，并交由 Pi 保存全局默认';
            } catch (error) {
                if (epoch !== this.epoch) return;
                this.modeStatus.textContent = `${error.message}；请核对设置，未自动重试`;
        ...
        static history(message) {
            ...
            summary.append(node('span', '手动 Shell'), node('code', message.command || ''), node('span', message.cancelled ? '已停止' : `exit ${message.exitCode ?? '--'}`));
            const flags = node('p', `${message.excludeFromContext ? '不加入模型上下文 · 原生记录保留' : '可加入模型上下文'}${message.truncated ? ' · 原生输出已截断' : ''}`, 'pi-shell-detail');
            const output = node('pre', message.output || '无输出'); output.tabIndex = 0;
            el.append(summary, flags, output);
            if (message.fullOutputPath) el.append(node('p', `原生完整输出路径：${message.fullOutputPath}`, 'pi-shell-detail'));
            return el;
        }
    }
