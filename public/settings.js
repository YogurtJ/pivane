document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadModels(force = false) {
        ...
        elements.providerList.innerHTML = '<div class="pi-list-state"><span class="pi-spinner"></span>正在读取 Provider</div>';
        elements.modelList.innerHTML = '<div class="pi-list-state"><span class="pi-spinner"></span>正在读取模型目录</div>';
    ...
    function authSourceLabel(provider) {
        const labels = {
            ...
            environment: provider.authLabel || '环境变量',
            models_json_key: 'models.json',
            models_json_command: 'models.json command',
            fallback: 'Provider 配置',
            runtime: '临时 Key'
        };
        return labels[provider.authSource] || provider.authSource || '未配置';
    }
    ...
    function providerControls(provider) {
        ...
        return `<article class="settings-row provider-row" data-provider-id="${escapeHtml(provider.id)}">
            ...
            <div class="settings-row-main">
                <div class="settings-row-title"><strong>${provider.configured ? '已配置' : '尚未连接'}</strong><span class="settings-badge">${custom?.api ? '自定义配置' : 'Pi 内置'}</span></div>
                <div class="settings-row-meta"><span>${escapeHtml(authSourceLabel(provider))}</span><span>${escapeHtml([methods.apiKey ? 'API Key' : '', methods.oauth ? 'OAuth' : ''].filter(Boolean).join(' · ') || '使用服务器环境认证')}</span></div>
            </div>
            <div class="settings-row-actions">
                ${methods.apiKeyLogin ? '<button type="button" data-action="key"><i class="fa-solid fa-key"></i> API Key 配置</button>' : ''}
                ${methods.oauth && state.modelSnapshot.providerLogin ? `<button type="button" data-action="oauth">${escapeHtml(methods.oauthLabel || 'OAuth 登录')}</button>` : ''}
                ${provider.storedCredential ? '<button type="button" data-action="logout">移除凭据</button>' : ''}
                ${custom?.api ? '<button type="button" data-action="edit" aria-label="编辑供应商">编辑供应商</button><button type="button" data-action="add-model">添加模型</button><button type="button" class="danger" data-action="delete">删除配置</button>' : ''}
            </div>
        </article>`;
    }
    ...
    function populateModelProviderFilter() {
        ...
        elements.modelProvider.innerHTML = '<option value="">全部 Provider</option>' + providers.map(provider => `<option value="${escapeHtml(provider.id)}">${escapeHtml(provider.name)}</option>`).join('');
    ...
    function populateMediaAgentControls() {
        ...
        elements.mediaAgentCurrent.textContent = current.provider && current.modelId ? `${current.provider}/${current.modelId}` : '跟随 Pi 默认模型';
    }
    ...
    async function saveMediaAgentModel(button) {
        ...
        if (!provider || !modelId) return toast('没有可用的模块 Agent 模型', 'error');
    ...
    function renderModels() {
        ...
        if (!state.modelGroups.size) {
            elements.modelList.innerHTML = '<div class="settings-empty">没有匹配的供应商或模型；可以关闭“仅可用 / 仅已配置”筛选。</div>';
            return;
        }
        ...
        elements.modelList.innerHTML = `
            <div class="settings-list-caption">${providers.length} 个 Provider · ${models.length} 个模型</div>
            ${providers.map(provider => {
                ...
                return `<details class="settings-model-group" ${open ? 'open' : ''}>
                    <summary data-provider="${escapeHtml(provider)}">
                        ...
                        <span class="settings-model-group-count">${configured.has(provider) ? '已配置 · ' : ''}${state.modelGroups.get(provider).length} 个模型</span>
                    </summary>
        ...
        group.querySelector('.settings-provider-models').innerHTML = visible.map(model => {
            ...
            return `
                <article class="settings-row model-row" data-provider-id="${escapeHtml(model.provider)}" data-model-id="${escapeHtml(model.id)}">
                    ...
                    <div class="settings-row-main">
                        <div class="settings-row-title"><strong title="${escapeHtml(model.name)}">${escapeHtml(model.name)}</strong>${isDefault ? '<span class="settings-badge primary">默认</span>' : ''}${model.reasoning ? '<span class="settings-badge">Reasoning</span>' : ''}</div>
                        <div class="settings-row-meta"><code title="${escapeHtml(`${model.provider}/${model.id}`)}">${escapeHtml(`${model.provider}/${model.id}`)}</code><span>${formatTokens(model.contextWindow)} context</span><span>${model.input.includes('image') ? '文本 + 图片' : '文本'}</span></div>
                        <div class="settings-row-meta"><span>${model.available ? '可用' : '未认证'}</span>${model.thinkingLevels ? `<span>Thinking: ${escapeHtml(model.thinkingLevels.join(' / '))}</span><span>默认: ${escapeHtml(defaults.modelThinkingLevels?.[`${model.provider}/${model.id}`] || '跟随全局')}</span>` : ''}</div>
                    </div>
                    <div class="settings-row-actions">
                        ${state.modelSnapshot.modelThinking ? '<button type="button" data-action="thinking">Thinking</button>' : ''}
                        <button type="button" data-action="test" ${model.available ? '' : 'disabled'}><i class="fa-solid fa-vial"></i> 测试</button>
                        ${isDefault ? '' : '<button type="button" data-action="default"><i class="fa-solid fa-star"></i> 设为默认</button>'}
                        ${editable ? '<button type="button" data-action="edit" aria-label="编辑模型"><i class="fa-solid fa-pen"></i></button><button type="button" class="danger" data-action="delete" aria-label="删除模型"><i class="fa-solid fa-trash"></i></button>' : ''}
                    </div>
                </article>`;
        }).join('') + (!models.length ? '<p class="settings-empty">此供应商暂无匹配模型。完成连接后可点击“刷新目录”。</p>' : '') + (pages > 1 ? `
            <div class="settings-model-pagination" role="group" aria-label="模型分页">
                <button class="settings-secondary-button" type="button" data-model-page="previous" ${page === 0 ? 'disabled' : ''}>上一页</button>
                <span>${page + 1} / ${pages} 页 · ${models.length} 项</span>
                <button class="settings-secondary-button" type="button" data-model-page="next" ${page === pages - 1 ? 'disabled' : ''}>下一页</button>
            </div>` : '');
    }
    ...
    async function openProviderLogin(providerId, method) {
        ...
        openEditor(`${provider?.name || providerId} · ${method === 'oauth' ? 'OAuth 登录' : 'API Key 配置'}`, `
            <div class="settings-form settings-login">
                <p>按 Pi 提供的步骤完成连接，凭据由 Pi 保存。关闭此窗口会取消尚未完成的登录。</p>
                ${method === 'oauth' ? '<p>在手机或其他电脑上授权时，localhost 回调可能无法打开；可将地址栏中的完整回调地址粘贴到下方 Pi 提供的输入框。</p>' : ''}
                <div id="settings-login-status" role="status" aria-live="polite">正在启动登录…</div>
                <div id="settings-login-events"></div><div id="settings-login-prompts"></div>
                <p id="settings-login-error" role="alert"></p>
                <button type="button" class="settings-secondary-button" data-editor-cancel>取消 / 关闭</button>
            </div>`);
        ...
        try {
            ...
            if (state.login === login) $('settings-login-error').textContent = `${error.message}。若连接中断，未完成的登录最多保留 10 分钟后失效。`;
        }
    }
    ...
    function updateLogin(login, snapshot) {
        ...
        const labels = { starting: '正在启动…', waiting: '请完成下面的授权步骤', cancelling: '正在取消，请核对最终凭据状态', success: '连接成功，凭据已保存', committed: '凭据已保存，请刷新检查', cancelled: '登录已取消', expired: '登录已过期', error: '登录未完成' };
        $('settings-login-status').textContent = snapshot.message || labels[snapshot.status] || snapshot.status;
        ...
        const link = (parent, url, label) => {
            try {
                ...
                const anchor = document.createElement('a'); anchor.href = parsed.href; anchor.textContent = label || '打开授权页面';
        ...
        for (const event of snapshot.events || []) {
            ...
            if (event.type === 'auth_url') { item.textContent = event.instructions || ''; link(item, event.url); }
                ...
                item.append('设备授权码：', code, ' '); link(item, event.verificationUri, '打开设备授权页面');
        ...
        for (const prompt of snapshot.prompts || []) {
            ...
            const submit = document.createElement('button'); submit.type = 'submit'; submit.className = 'settings-primary-button'; submit.textContent = '继续';
            ...
            form.addEventListener('submit', async event => {
                ...
                try {
                    ...
                    if (state.login === login) $('settings-login-error').textContent = `${error.message}；请等待状态更新，或取消后核对凭据。不会自动重复提交。`;
                }
            });
    ...
    function pollLogin(login) {
        ...
        login.timer = setTimeout(async () => {
            try {
                ...
                if (state.login === login) $('settings-login-error').textContent = `暂时无法读取登录状态：${error.message}`;
            }
            pollLogin(login);
        }, 750);
    }
    ...
    function openThinkingEditor(provider, modelId) {
        ...
        openEditor(`${model.name} · Thinking`, `
            <form id="settings-thinking-default-form" class="settings-form">
                <p>可用等级：${escapeHtml(model.thinkingLevels.join(' / '))}。默认值用于新的会话；恢复已有会话时保留其思考状态。</p>
                <label><span>此模型默认思考等级</span><select name="level"><option value="">跟随全局（${escapeHtml(snapshot.preferences.defaultThinkingLevel || 'Pi 默认')}）</option>${model.thinkingLevels.map(level => `<option value="${escapeHtml(level)}">${escapeHtml(level)}</option>`).join('')}</select></label>
                <button class="settings-primary-button" type="submit">保存默认值</button>
            </form>
            <details class="settings-thinking-advanced"><summary>高级：模型支持的等级与参数映射</summary>
                <p>仅按实际服务能力修改。禁用的等级将从 Pi 选择器隐藏；映射值会传给供应商。保存能力后重新打开窗口选择默认值。</p>
                <form id="settings-thinking-map-form" class="settings-form">
                    ${keys.map(level => `<div class="settings-thinking-map-row" data-level="${escapeHtml(level)}"><label><span>${escapeHtml(level)}</span><select aria-label="${escapeHtml(level)} 支持方式"><option value="inherit">使用 Pi 定义</option><option value="disabled">不支持</option><option value="mapped">自定义映射</option></select></label><label><span>供应商参数值</span><input aria-label="${escapeHtml(level)} 参数值" maxlength="100"></label></div>`).join('')}
                    <div class="settings-form-actions"><button type="button" id="settings-thinking-reset">恢复 Pi 原始能力</button><button class="settings-primary-button" type="submit">保存能力</button></div>
                </form>
        ...
        const save = async (payload, button) => {
            ...
            try {
                ...
                toast('Thinking 配置已保存；新 runtime 生效，运行中的会话保留当前状态', 'success', 6500);
            } catch (error) {
    ...
    async function saveApiKey(providerId, apiKey, button) {
        ...
        try {
            ...
            toast('API Key 已保存到 Pi credential store；重新打开 runtime 后生效', 'success', 6500);
        } finally {
    ...
    function openApiKeyEditor(providerId) {
        const provider = state.modelSnapshot.providers.find(item => item.id === providerId);
        openEditor(`设置 ${provider?.name || providerId} API Key`, `
            <form id="settings-api-key-form" class="settings-form">
                <div class="settings-secret-note"><i class="fa-solid fa-shield-halved"></i><span>凭据由 Pi 保存，现有值不会回显。请在受信网络中配置。</span></div>
                <label><span>API Key</span><input id="settings-api-key-input" type="password" autocomplete="new-password" required></label>
                <div class="settings-form-actions"><button type="button" data-editor-cancel>取消</button><button class="settings-primary-button" type="submit"><i class="fa-solid fa-floppy-disk"></i> 保存</button></div>
            </form>
        `);
    ...
    async function removeCredential(providerId, button) {
        if (!confirm('移除 Pi credential store 中这个 Provider 的凭据？环境变量或 models.json 中的配置不会被删除。')) return;
        setBusy(button, true);
        try {
            const result = await apiFetch(`/api/pi/settings/providers/${encodeURIComponent(providerId)}/credential`, { method: 'DELETE' });
            state.modelSnapshot = null;
            await loadModels(true);
            toast(result.stillConfigured ? `已移除存储凭据；Provider 仍由 ${result.authSource} 配置` : '凭据已移除', 'success');
        } catch (error) { toast(error.message, 'error'); }
        finally { setBusy(button, false); }
    }
    ...
    function openProviderEditor(provider) {
        ...
        openEditor(editing ? `编辑 ${provider.id}` : '新增自定义 Provider', `
            <form id="settings-provider-form" class="settings-form">
                <div class="settings-form-grid">
                    ...
                    <label><span>API 类型</span><select name="api"><option value="openai-completions">OpenAI Chat Completions</option><option value="openai-responses">OpenAI Responses</option><option value="anthropic-messages">Anthropic Messages</option><option value="google-generative-ai">Google Generative AI</option></select></label>
                    ...
                    <label class="settings-checkbox-field"><input name="authHeader" type="checkbox" ${provider?.authHeader ? 'checked' : ''}><span>自动添加 Authorization: Bearer</span></label>
                </div>
                <div class="settings-form-actions"><button type="button" data-editor-cancel>取消</button><button class="settings-primary-button" type="submit">保存 Provider</button></div>
            </form>
        `);
        ...
        form.addEventListener('submit', async event => {
            ...
            try {
                ...
                toast('自定义 Provider 已保存；请设置 API Key 并重新打开 runtime', 'success', 6500);
            } catch (error) { toast(error.message, 'error'); }
    ...
    async function deleteProvider(providerId, button) {
        if (!confirm(`删除自定义 Provider “${providerId}”及其 models.json 模型配置？`)) return;
        setBusy(button, true);
        try {
            await apiFetch(`/api/pi/settings/custom-providers/${encodeURIComponent(providerId)}`, { method: 'DELETE' });
            state.modelSnapshot = null; await loadModels(true); toast('自定义 Provider 已删除', 'success');
        } catch (error) { toast(error.message, 'error'); }
        finally { setBusy(button, false); }
    }
    ...
    function openModelEditor(providerId, model) {
        ...
        if (!customProviders.length) return toast('请先创建自定义 Provider', 'error');
        openEditor(model ? `编辑 ${model.id}` : '新增自定义模型', `
            <form id="settings-model-form" class="settings-form">
                <div class="settings-form-grid">
                    ...
                    <label class="wide"><span>显示名称</span><input name="name" value="${escapeHtml(model?.name || '')}" placeholder="默认使用 Model ID"></label>
                    ...
                    <label><span>最大输出 Tokens</span><input name="maxTokens" type="number" min="1" max="1000000" value="${model?.maxTokens || 16384}" required></label>
                    <label class="settings-checkbox-field"><input name="reasoning" type="checkbox" ${model?.reasoning ? 'checked' : ''}><span>支持 reasoning</span></label>
                    <label class="settings-checkbox-field"><input name="imageInput" type="checkbox" ${model?.input?.includes('image') ? 'checked' : ''}><span>支持图片输入</span></label>
                </div>
                <div class="settings-form-actions"><button type="button" data-editor-cancel>取消</button><button class="settings-primary-button" type="submit">保存模型</button></div>
            </form>
        `);
        ...
        form.addEventListener('submit', async event => {
            event.preventDefault();
            const submit = form.querySelector('[type=submit]'); setBusy(submit, true, '保存');
            const data = Object.fromEntries(new FormData(form));
            ...
            try {
                ...
                closeEditor(); state.modelSnapshot = null; await loadModels(true); toast('自定义模型已保存', 'success');
            } catch (error) { toast(error.message, 'error'); }
            finally { setBusy(submit, false); }
        });
    }
    ...
    async function deleteModel(providerId, modelId, button) {
        if (!confirm(`删除模型 “${providerId}/${modelId}”？`)) return;
        setBusy(button, true);
        try {
            ...
            state.modelSnapshot = null; await loadModels(true); toast('模型已删除', 'success');
        } catch (error) { toast(error.message, 'error'); }
        finally { setBusy(button, false); }
    }
    ...
    async function setDefaultModel(provider, modelId, button) {
        ...
        try {
            ...
            renderModels();
            toast('默认模型已更新；新 runtime 将使用此模型', 'success');
        } catch (error) { toast(error.message, 'error'); }
        finally { setBusy(button, false); }
    }
    ...
    async function testModel(provider, modelId, button) {
        ...
        elements.modelResult.textContent = `正在测试 ${provider}/${modelId}...`;
        try {
            ...
            elements.modelResult.innerHTML = `<strong>连接成功 · ${(result.latencyMs / 1000).toFixed(2)}s</strong><span>${escapeHtml(result.text || '(空回复)')}</span>`;
        } catch (error) {
            elements.modelResult.className = 'settings-result error';
            elements.modelResult.innerHTML = `<strong>测试失败</strong><span>${escapeHtml(error.message)}</span>`;
        } finally { setBusy(button, false); }
    }
    ...
    async function refreshModelCatalog(button) {
        ...
        try {
            ...
            state.modelSnapshot = null; await loadModels(true);
            toast(result.errors.length ? `目录已刷新，${result.errors.length} 个 Provider 失败` : '模型目录已刷新', result.errors.length ? 'info' : 'success');
        } catch (error) { toast(error.message, 'error'); }
        finally { setBusy(button, false); }
    }
    ...
    async function loadResources(force = false) {
        ...
        elements.packageList.innerHTML = '<div class="pi-list-state"><span class="pi-spinner"></span>正在解析 Packages</div>';
        elements.skillList.innerHTML = '<div class="pi-list-state"><span class="pi-spinner"></span>正在读取 Skills</div>';
    ...
    function renderPackages() {
        ...
        elements.packageList.innerHTML = packages.length ? packages.map(item => `
            <article class="settings-row package-row" data-package-source="${escapeHtml(item.source)}">
                ...
                <div class="settings-row-main"><div class="settings-row-title"><strong>${escapeHtml(item.source)}</strong>${item.filtered ? '<span class="settings-badge">已过滤</span>' : ''}</div><div class="settings-row-meta"><span>${escapeHtml(item.scope)}</span><span>${item.installed ? '已安装' : '配置存在但未安装'}</span></div></div>
                <div class="settings-row-actions"><button type="button" data-action="update"><i class="fa-solid fa-arrows-rotate"></i> 更新</button><button type="button" class="danger" data-action="remove"><i class="fa-solid fa-trash"></i> 删除</button></div>
            </article>
        `).join('') : '<div class="settings-empty">尚未安装 Pi Package</div>';
    ...
    async function runPackageAction(action, source, button) {
        if (action === 'remove' && !confirm(`删除 Package “${source}”？`)) return;
        if (action === 'install' && !confirm(`安装并信任 Package “${source}”？Packages 可执行任意代码。`)) return;
        setBusy(button, true, action === 'install' ? '安装中' : action === 'remove' ? '删除中' : '更新中');
        ...
        try {
            ...
            elements.packageProgress.textContent = result.progress.map(item => item.message).filter(Boolean).slice(-3).join(' · ') || `${action} 完成`;
            state.resourceSnapshot = null; await loadResources(true);
            toast('Package 配置已更新；重新打开 runtime 后生效', 'success', 6500);
        } catch (error) {
    ...
    function renderSkills() {
        ...
        elements.skillList.innerHTML = skills.length ? skills.map(skill => `
            <article class="settings-row skill-row" data-skill-name="${escapeHtml(skill.name)}">
                ...
                <div class="settings-row-main"><div class="settings-row-title"><strong>${escapeHtml(skill.name)}</strong>${skill.disableModelInvocation ? '<span class="settings-badge">仅手动</span>' : ''}</div><p>${escapeHtml(skill.description)}</p><div class="settings-row-meta"><span>${escapeHtml(skill.scope)}</span><span>${escapeHtml(skill.source)}</span><code title="${escapeHtml(skill.filePath)}">${escapeHtml(skill.filePath)}</code></div></div>
        ...
        `).join('') : '<div class="settings-empty">没有发现 Skill</div>';
    ...
    async function deleteSkill(name, button) {
        if (!confirm(`删除用户 Skill “${name}”？`)) return;
        setBusy(button, true);
        try {
            await apiFetch(`/api/pi/settings/skills/${encodeURIComponent(name)}`, { method: 'DELETE' });
            state.resourceSnapshot = null; await loadResources(true); toast('Skill 已移入回收站', 'success');
        } catch (error) { toast(error.message, 'error'); }
        finally { setBusy(button, false); }
    }
    ...
    elements.skillCommands.addEventListener('change', async () => {
        try {
            ...
            toast('Skill command 设置已保存；重新打开 runtime 后生效', 'success');
