(() => {
    ...
    const stages = { progress: translateUi("过程回复"), final: translateUi("最终回复"), pending: translateUi("回复中"), stopped: translateUi("已停止"), error: translateUi("回复失败") };
    function timestamp(value) {
        ...
        time.dateTime = date.toISOString(); time.title = date.toLocaleString(globalThis.PiI18n?.locale); time.setAttribute('aria-label', time.title);
    ...
    function fillHeading(heading, row) {
        ...
        const label = row.kind === 'user' ? translateUi("用户问题") : row.kind === 'tool' ? translateUi("工具记录")
            : row.kind === 'summary' ? row.summaryType === 'compaction' ? translateUi("上下文压缩") : row.summaryType === 'branch_summary' ? translateUi("分支摘要") : translateUi("摘要") : 'AI';
        role.append(icon(row.kind === 'user' ? 'user' : row.kind === 'tool' ? 'wrench' : row.kind === 'summary' ? 'file-lines' : 'robot'), document.createTextNode(label));
        heading.append(role);
        if (row.kind === 'assistant') { const badge = el('span', stages[row.replyStage] || translateUi("回复")); badge.className = 'pi-tree-stage'; heading.append(badge); }
    ...
    class PiSessionTreeView {
        ...
        sync() {
            ...
            $('pi-tree-continue').textContent = selected?.stale ? translateUi("重新核对继续位置") : atCurrent ? translateUi("当前已在这里继续") : selected?.navigation === 'before' ? translateUi("回到此问题前，修改后继续") : translateUi("从这里继续");
            $('pi-tree-continue').disabled = !ready || c.busy || busy || !selected?.revision || atCurrent;
            $('pi-tree-continue').title = c.busy || busy ? translateUi("等待当前任务结束后可继续") : translateUi("切换对话位置，不自动发送消息");
        ...
        open(mode = 'tree') {
            if (mode === 'tree' && !this.available()) { this.host.toast(translateUi("当前实例尚未启用会话树，请连接持久线程"), 'info'); return false; }
        ...
        async load(focus, offset = 0) {
            ...
            this.loading = true; this.sync(); $('pi-tree-status').textContent = translateUi("正在读取会话树…");
            try {
                ...
                $('pi-tree-status').textContent = data.total ? translateUi("{0}{1}–{2} / {3} 个对话节点", data.currentId ? '' : translateUi("当前在第一条问题之前 · "), data.offset + 1, data.offset + data.rows.length, data.total) : translateUi("还没有可查看的对话记录");
        ...
        render() {
            ...
            for (const row of this.data.rows) {
                ...
                toggle.setAttribute('aria-label', row.collapsed ? translateUi("展开后续对话") : translateUi("收起后续对话")); toggle.setAttribute('aria-expanded', String(!row.collapsed));
                ...
                if (row.current) { const badge = el('span', translateUi("当前位置")); badge.className = 'pi-tree-current-badge'; context.append(badge); }
                if (!row.inCurrentBranch) context.append(el('span', translateUi("其他分支")));
                if (row.childCount > 1) context.append(el('span', translateUi("{0} 条后续路线", row.childCount)));
                if (row.collapsed) context.append(el('span', translateUi("后续已收起")));
                if (context.childNodes.length) button.append(context);
                button.title = row.parentPreview ? translateUi("接在：{0}", row.parentPreview) : translateUi("对话起点");
        ...
        apply(value) {
            ...
            this.value = value;
            const names = { preparing: translateUi("正在核对导航位置…"), navigating: translateUi("正在切换对话位置…"), summarizing: translateUi("正在生成离开分支的摘要…"), retrying: translateUi("摘要服务正在重试…"), stopping: translateUi("正在取消，请等待结果…"), done: translateUi("已切换对话位置，可在主输入框继续。"), cancelled: translateUi("导航已取消。"), error: translateUi("导航未完成，请重新预览目标并核对当前位置。"), uncertain: translateUi("导航结果不确定，请重新连接并核对当前位置；未自动重试。") };
        ...
        async navigate() {
            ...
            if (!window.confirm(translateUi("{0}\n原分支会保留，项目文件保持当前版本。{1}", selected.navigation === 'before' ? translateUi("回到这个问题之前，准备修改后继续？") : translateUi("从这条记录之后继续对话？"), $('pi-tree-summarize').checked ? translateUi("\n将调用模型生成离开分支的摘要。") : ''))) return;
            ...
            try {
                ...
                selected.stale = true;
                this.host.toast(data.cancelled ? translateUi("导航已取消") : this.draft ? translateUi("已切换；原草稿保留，可按需追加原问题") : translateUi("已切换，请核对后继续"), 'info');
            } catch (e) { if (this.history.current(key, generation)) this.host.toast(translateUi("{0}；请核对当前位置，未自动重试", e.message), 'error'); }
        ...
        async cancel() {
            ...
            catch (e) { if (this.history.current(key, generation)) this.host.toast(translateUi("{0}；请等待导航最终状态", e.message), 'error'); }
        }
    }
