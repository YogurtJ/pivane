(() => {
    ...
    const scopes = [['global', '所有项目'], ['project', '当前项目']];
    const groups = [
        ['messages', '消息与模型', '消息投递方式、常用模型范围', 'fa-comment-dots', k => ['steeringMode', 'followUpMode', 'enabledModels'].includes(k)],
        ['tools', '工具', '默认启用的内置工具与项目继承', 'fa-toolbox', k => k === 'defaultTools'],
        ['trust', '项目信任', '未决项目的全局默认策略', 'fa-shield-halved', k => k === 'defaultProjectTrust'],
        ['context', '上下文', '自动压缩与保留内容', 'fa-layer-group', k => k.startsWith('compaction.')],
        ['images', '图片', '自动缩放与图片输入', 'fa-image', k => k.startsWith('images.')],
        ['connection', '连接与重试', '连接方式、失败重试与超时', 'fa-plug', k => k === 'transport' || k.startsWith('retry.') || k.endsWith('TimeoutMs')],
        ['privacy', '隐私与诊断', '安装遥测与环境覆盖', 'fa-shield-halved', k => k === 'enableInstallTelemetry'],
        ['other', '其他选项', '其他原生设置', 'fa-sliders', () => true]
    ];
    ...
    function create({ apiFetch, currentCwd }) {
        ...
        const send = (url, method, data) => {
            if (data.cwd && data.cwd !== contextCwd()) throw new Error('项目已切换，请重新打开设置');
        ...
        function trustLink(cwd) { return button('管理项目信任', '', () => window.dispatchEvent(new CustomEvent('pi:project-trust', { detail: { cwd } }))); }
        ...
        function toolField(spec, current, scope, disabled) {
            ...
            const mode = select([['inherit', scope === 'project' ? '继承全局' : 'Pi 默认'], ['custom', '自定义']], saved === null ? 'inherit' : 'custom', 'native-tools-mode');
            mode.disabled = disabled;
            wrapper.append(label('工具选择方式', mode));
            const list = node('fieldset', undefined, { class: 'native-tool-options' });
            list.append(node('legend', '内置工具'));
            ...
            for (const name of [...new Set([...spec.choices, ...inherited, ...selected])]) {
                ...
                const caption = node('span', name + (!spec.choices.includes(name) ? '（当前版本不支持）' : name === 'powershell' ? '（需 PowerShell）' : ''));
            ...
            const update = () => {
                ...
                summary.textContent = mode.value === 'custom' ? chosen.length ? `已选择 ${chosen.length} 个内置工具` : '未启用任何内置工具；扩展和自定义工具仍可启用。' : scope === 'project' ? '移除项目覆盖，使用全局配置或 Pi 默认。' : `Pi 默认：${spec.defaults.join('、')}`;
            };
        ...
        function settingsForm(snapshot) {
            ...
            for (const [id, title, description, icon, matches] of groups) {
                ...
                heading.append(node('i', '', { class: `fa-solid ${icon}`, 'aria-hidden': 'true' }), caption, node('span', count ? `${count} 项自定义` : '默认', { class: 'native-setting-count' }));
                ...
                for (const [key, spec] of fields) {
                    ...
                    if (spec.type === 'tools') {
                        ...
                        const names = { ask: '询问（Pi 默认）', always: '默认信任', never: '默认不信任', all: '全部投递', 'one-at-a-time': '逐条投递', auto: '自动选择' };
                        const choices = spec.choices?.map(v => [v, names[v] || v]) || [['true', '开启'], ['false', '关闭']];
                        field = select([['', scope === 'project' ? '继承全局' : 'Pi 默认'], ...choices], saved === null ? '' : String(saved));
                    } else {
                        ...
                        if (spec.type === 'lines') { field.rows = 3; field.placeholder = '每行一个模型名称或匹配模式'; }
                        else field.placeholder = current.value === null ? '使用默认值' : String(current.value);
                    ...
                    const row = tool ? tool.wrapper : label(spec.label, field);
                    const value = key === 'defaultProjectTrust' ? ({ ask: '询问', always: '默认信任', never: '默认不信任' }[current.value] || current.value) : spec.type === 'tools' ? current.value?.join('、') || '未启用任何内置工具' : typeof current.value === 'boolean' ? current.value ? '开启' : '关闭' : Array.isArray(current.value) ? current.value.join('、') || '未限定' : current.value ?? '默认';
                    row.append(node('small', `当前配置：${value} · ${{ project: '项目', global: '全局', default: 'Pi 默认' }[current.source]}`)); grid.append(row);
                }
                if (id === 'tools') {
                    grid.append(node('p', '只选择模型初始可用的内置工具。扩展和自定义工具仍可启用；这不是只读或安全模式，也不控制手动 ! Shell。当前实际工具可在会话详情 → 当前加载的资源中查看。', { class: 'native-setting-help' }));
                    const view = button('查看当前实例的工具', 'native-tools-runtime', () => {
                        const resources = $('pi-loaded-resources');
                        if (!resources || resources.hidden) return status('native-status', '请先打开会话，再查看当前实例的工具。');
                    ...
                    view.title = view.disabled ? '打开会话后可查看' : '读取当前实例实际启用的工具，不应用配置';
                    grid.append(view);
                }
                if (id === 'trust') {
                    grid.append(node('p', '仅作用于没有其他适用决定的项目，已有项目或父目录的允许／拒绝仍有效。Web 的 RPC 不弹出启动询问：选择“询问”时，未决项目的受信资源暂不加载，可从项目菜单或 /trust 授权。', { class: 'native-setting-help' }));
                    if (currentCwd()) grid.append(node('p', `当前项目按配置推导：${snapshot.trust.effective ? '已信任' : '未信任'}。${snapshot.trust.override !== null ? '运行环境已固定信任决定。' : snapshot.trust.savedPath ? '信任决定来自：' + snapshot.trust.savedPath + '。' : '使用全局默认策略。'} 当前运行实例的实际状态在会话详情查看，扩展也可能参与信任决定。`, { class: 'native-setting-help' }), trustLink(snapshot.cwd));
                }
                if (id === 'privacy') {
                    const env = snapshot.environment || {};
                    grid.append(node('p', `Web 会话已关闭版本检查。${env.offline ? '当前启用离线启动。' : ''}${env.telemetry !== null && env.telemetry !== undefined ? '环境变量将遥测固定为' + (env.telemetry ? '开启。' : '关闭。') : '遥测独立于版本检查。'}`));
                }
                section.append(heading, grid); form.append(section);
            }
            const save = node('button', '保存修改', { type: 'submit', id: 'native-settings-save', class: 'settings-primary-button' });
            save.disabled = scope === 'project' && !snapshot.trust.effective;
            const footer = node('div', undefined, { class: 'native-save-row' }); footer.append(node('small', '修改后保存，新运行实例生效；已有线程请在任务结束后 /quit，再重新打开。'), save);
            footer.append(button('查看当前实例的生效状态', 'native-show-runtime-config', () => {
                if (!$('pi-loaded-resources') || $('pi-loaded-resources').hidden) return status('native-status', '请先打开线程，再查看运行实例。');
            ...
            form.onsubmit = e => {
                ...
                if (!Object.keys(values).length) return status('native-status', '没有需要保存的修改');
                ...
                void act(save, async () => {
                    ...
                    status('native-status', '已保存，新运行实例生效。已有线程请在任务结束后 /quit，再重新打开。');
                    ...
                    if (committed && valid(n, cwd) && config === snapshot) {
                        form.replaceChildren(node('p', '已保存，但配置读取失败。请点击刷新核对，勿重复提交。'));
                        status('native-status', '已保存，但配置读取失败。请刷新核对。');
                    }
                });
            };
        }
        async function loadNative(n, cwd) {
            panel.replaceChildren(node('p', '正在读取配置…'));
            ...
            caption.append(node('h3', 'Pi 配置'), node('p', currentCwd() ? '通常无需修改。需要时展开对应分类即可。' : '尚未选择项目，可先管理全局配置；选择项目后可设置项目覆盖。'));
            heading.append(caption, button('刷新', 'native-refresh', () => open('native')));
            ...
            const updateScope = () => { hint.replaceChildren(node('span', scope.value === 'global' ? '作为所有项目的默认配置' : cwd)); if (scope.value === 'project' && !config.trust.effective) hint.append(node('span', '信任此项目后可保存项目配置。'), trustLink(cwd)); settingsForm(config); };
            scope.addEventListener('change', updateScope); scopeRow.append(label('应用范围', scope), hint);
        ...
        async function loadSkills(n, cwd, scope = $('native-skills-scope')?.value || 'global') {
            ...
            try {
                ...
                const choice = select(availableScopes(), scope, 'native-skills-scope'); choice.setAttribute('aria-label', 'Skill 应用范围');
                choice.addEventListener('change', () => void loadSkills(n, cwd, choice.value));
                const search = node('input', undefined, { type: 'search', placeholder: '搜索 Skill', 'aria-label': '搜索 Skill', id: 'native-skills-search' });
                const toolbar = node('div', undefined, { class: 'native-skills-toolbar' });
                toolbar.append(search, choice, button('刷新', 'native-skills-refresh', () => loadSkills(n, cwd, choice.value)));
                const note = node('p', '停用会保留文件。选择会保存到所选范围，重新加载资源或下次打开会话后生效。', { class: 'native-help' });
                skillPanel.append(toolbar, note);
                const disabled = scope === 'project' && !snapshot.trust.effective;
                if (disabled) { const hint = node('div', undefined, { class: 'native-scope-hint' }); hint.append(node('span', '当前项目未信任，暂不能修改此范围。'), trustLink(cwd)); skillPanel.append(hint); }
                ...
                const render = () => {
                    ...
                    for (const skill of found.slice(0, limit)) {
                        ...
                        title.append(node('strong', skill.name), node('span', skill.enabled ? '已启用' : '已停用', { class: 'native-skill-badge' })); main.append(title);
                        if (skill.description) main.append(node('p', skill.description));
                        const info = node('details', undefined, { class: 'native-skill-source' }); info.append(node('summary', skill.scope === 'project' ? '来自项目 · 查看来源' : '全局资源 · 查看来源'), node('p', skill.source), node('code', skill.path)); main.append(info);
                        const actions = node('div', undefined, { class: 'native-skill-actions' });
                        async function change(state, b) {
                            if (!active()) return;
                            if (state === 'on' && !confirm(`启用 ${skill.name}？Skill 的指令和脚本会影响 Agent 行为。`)) return;
                            ...
                            await act(b, async () => {
                                ...
                                if (valid(n, cwd) && $('native-skills-scope')?.value === scope) status('native-skills-status', refreshed ? '已保存。重新加载资源或下次打开会话后生效。' : '已保存，但列表读取失败。请刷新核对，不要重复提交。');
                            }, 'native-skills-status');
                            for (const c of controls) if (c.isConnected) c.disabled = disabled && c.closest('.native-skill-row') !== null;
                        }
                        const toggle = button(skill.enabled ? '停用' : '启用', '', () => change(skill.enabled ? 'off' : 'on', toggle)); toggle.className = 'native-skill-toggle'; toggle.setAttribute('aria-label', `${skill.enabled ? '停用' : '启用'} ${skill.name}`); toggle.disabled = disabled; actions.append(toggle);
                        if (scope === 'project' && skill.override !== 'inherit') { const reset = button('恢复继承', '', () => change('inherit', reset)); reset.disabled = disabled; actions.append(reset); }
                        row.append(main, actions); list.append(row);
                    }
                    if (!found.length) list.append(node('p', query ? '没有匹配的 Skill' : '此范围没有发现 Skill。可以让 Agent 帮你安装或整理。', { class: 'settings-empty' }));
                    if (found.length > limit) list.append(button(`显示更多（剩余 ${found.length - limit} 项）`, '', () => { limit += 40; render(); }));
                };
                search.addEventListener('input', () => { limit = 40; render(); }); render(); return true;
            } catch (e) { if (active()) {
                if (!$('native-skills-status')) skillPanel.replaceChildren(node('p', '', { id: 'native-skills-status', role: 'status' }), button('重试读取', '', () => loadSkills(n, cwd, scope)));
                skillPanel.querySelector('.native-skills-list')?.replaceChildren(node('p', '暂时无法读取配置，请刷新核对。'));
        ...
        async function loadPackages(n, cwd, scope = currentCwd() ? $('native-resource-scope')?.value || 'project' : 'global') {
            ...
            packagePanel.replaceChildren(node('h3', 'Packages 与资源'));
            packagePanel.append(node('p', '管理资源配置；当前实际加载情况可在会话详情查看。安装或启用的扩展可执行代码，请先审查来源。'));
            const compatibility = node('details', undefined, { class: 'native-runtime-note' });
            compatibility.append(node('summary', '扩展在网页中的兼容范围'), node('p', '工具、命令、基础确认／选择／输入／编辑窗口，以及文字状态可通过 Pi RPC 使用。安装成功仅代表配置完成，不代表已验证网页兼容。'), node('p', '终端专用 custom 界面、编辑器、快捷键和组件式 widget 不会自动转换为网页；启动阶段的阻塞交互在当前上游 RPC 中受限。请优先使用声明支持 RPC 的扩展。')); packagePanel.append(compatibility);
            ...
            packagePanel.append(label('管理范围', choice));
            if (!snapshot.trust.effective && scope === 'project') { packagePanel.append(node('p', '当前项目未信任，项目写入已禁用；可切换所有项目或管理信任。'), trustLink(cwd)); }
            const disabled = scope === 'project' && !snapshot.trust.effective;
            const source = node('input', undefined, { placeholder: 'npm:package@version 或 git:https://…', 'aria-label': 'Package 来源' });
            const install = button('安装', 'native-package-install', () => packageAction(install, 'install', source.value)); install.disabled = disabled;
            ...
            async function packageAction(b, action, source) {
                if (!confirm(`${action} ${source}（${scope === 'global' ? '全局' : '项目'}）？包操作可能执行代码；失败后请核对配置，不自动重试。`)) return;
                await act(b, async () => { await send('/api/pi/settings/native/packages', 'POST', { cwd, scope, source, action, expectedRevision: snapshot.revision, confirmed: true }); if (valid(n, cwd) && $('native-resource-scope')?.value === scope) { await loadPackages(n, cwd, scope); status('native-resource-status', '操作完成；空闲时重新加载资源。'); } }, 'native-resource-status');
            }
            for (const pkg of snapshot.packages) {
                const row = node('div', undefined, { class: 'native-card' }); row.append(node('strong', pkg.source), node('p', `${pkg.scope === 'user' ? '全局' : '项目'} · ${pkg.installed ? '已安装' : '尚未安装'}`));
                if (pkg.scope === (scope === 'global' ? 'user' : 'project')) for (const [text, action] of [['更新', 'update'], ['移除', 'remove']]) { const b = button(text, '', () => packageAction(b, action, pkg.source)); b.disabled = disabled; row.append(b); }
                packagePanel.append(row);
            }
            const search = node('input', undefined, { type: 'search', placeholder: '筛选资源名称或来源', 'aria-label': '筛选资源' }); packagePanel.append(search);
            ...
            const render = () => {
                ...
                for (const r of found.slice(0, limit)) {
                    ...
                    const title = node('div'); title.append(node('strong', r.path.split('/').at(-1)), node('small', `${r.type} · ${r.scope} · ${r.enabled ? '配置启用' : '配置停用'}`), node('small', r.path));
                    const value = select([['inherit', '继承 / 默认'], ['on', '启用'], ['off', '停用']], scope === 'global' ? r.enabled ? 'on' : 'off' : r.override); value.setAttribute('aria-label', r.path + ' 开关'); value.disabled = disabled;
                    const save = button('保存', '', () => { if (!confirm(`将 ${r.path} 在${scope === 'global' ? '全局' : '项目'}设为${value.selectedOptions[0].textContent}？`)) return; void act(save, async () => { await send('/api/pi/settings/native/resources', 'PUT', { cwd, scope, resourceId: r.id, state: value.value, expectedRevision: snapshot.revision, confirmed: true }); if (valid(n, cwd) && $('native-resource-scope')?.value === scope) { await loadPackages(n, cwd, scope); status('native-resource-status', '已保存，空闲时重新加载资源。'); } }, 'native-resource-status'); });
                    save.disabled = disabled; row.append(title, value, save); list.append(row);
                }
                if (found.length > limit) list.append(button(`显示更多（剩余 ${found.length - limit} 项）`, '', () => { limit += 40; render(); }));
                if (!found.length) list.append(node('p', '没有匹配资源'));
            };
        ...
        async function open(tab) {
            ...
            try {
                ...
                if (!cwd) throw new Error('服务器没有可用的项目目录，请检查项目范围设置（PI_PROJECT_ROOTS）。');
                $('native-settings-nav').hidden = !capabilities.nativeSettings;
                if (tab === 'native') { if (!capabilities.nativeSettings) panel.textContent = '当前后端尚未启用 Pi 配置'; else await loadNative(n, cwd); }
